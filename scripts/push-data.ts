...
const tableFilters = args.filter((a) => a.startsWith('--table=')).map((a) => a.split('=')[1]);
const excludeFilters = args.filter((a) => a.startsWith('--exclude=')).map((a) => a.split('=')[1]);
...
if (!existsSync(wranglerDir)) {
	console.error(
		'❌ No .wrangler directory found. Has the local dev server been run at least once?'
	);
	process.exit(1);
}
...
for (const line of lines) {
	...
	if (insertMatch) {
		...
		if (ALWAYS_EXCLUDED.includes(currentTable) || excludeFilters.includes(currentTable)) {
		...
		const transformed = trimmed.replace(/^INSERT INTO/i, 'INSERT OR REPLACE INTO');
...
console.log(`\n📊 Data Summary:`);
for (const [table, stmts] of Object.entries(tableStatements).sort(([a], [b]) =>
	a.localeCompare(b)
)) {
...
for (const [table, stmts] of Object.entries(tableStatements).sort(([a], [b]) =>
	...
	try {
		execSync(`wrangler d1 execute ${dbName} --remote --file="${tableFile}"`, {
			cwd: projectRoot,
			stdio: 'pipe',
		});
		console.log(`   ✅ ${table}: ${stmts.length} row(s)`);
		...
		const sqlError =
			msg.match(/\[ERROR\]\s*(.+?)(?::\s*SQLITE_ERROR)?/)?.[1]?.trim() || msg.split('\n')[0];
	...
	try {
		if (!failedTables.some((f) => f.table === table)) unlinkSync(tableFile);
	} catch {}
}
