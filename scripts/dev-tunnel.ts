 ...
 *
 *   <label>-spacebot-<hostname>-<hash>.starspace.group  ->  localhost:<port>
 *
 ...
 *
 * Examples:
 *   bun run scripts/dev-tunnel.ts up                  # dev-spacebot-<host>-<hash>.starspace.group -> :4269
...
function ensureCloudflaredReady() {
	...
	if (!existsSync(join(CLOUDFLARED_DIR, 'cert.pem'))) {
		console.error(
			'❌ Not authenticated with Cloudflare yet. Run `cloudflared tunnel login` first (opens a browser), then re-run this script.'
		);
		process.exit(1);
	}
}
...
function ensureTunnel(): TunnelState {
	...
	if (existing) {
		...
		const stillExists = tunnels.some(
			(t) => t.id === existing.tunnelId && t.deleted_at === NOT_DELETED
		);
		if (stillExists) return existing;
		console.warn(
			`⚠️  Saved tunnel ${existing.tunnelName} (${existing.tunnelId}) no longer exists in your account — recreating.`
		);
	}
	...
	if (!created) {
		console.error(
			`❌ Created tunnel ${tunnelName} but couldn't find it in \`cloudflared tunnel list\` afterwards.`
		);
		process.exit(1);
	}
...
async function cmdUp(label: string, port: number) {
	...
	const ruleHostname = `${safeLabel}-${state.tunnelName}.${ZONE}`;
	const existingRule = state.ingress.find((r) => r.label === safeLabel);
	if (existingRule) {
		existingRule.port = port;
		// Refresh the hostname too, so a change to the naming scheme takes effect
		// on the next `up` instead of staying pinned to whatever was first saved.
		existingRule.hostname = ruleHostname;
	} else {
...
function cmdDown() {
	...
	stopRunningDaemon();
	console.log(
		'Tunnel daemon stopped. DNS routes and the tunnel itself are left in place — run `up` again to reconnect.'
	);
}
...
function cmdStatus() {
	...
	if (!state) {
		console.log(
			'No dev tunnel configured on this machine yet. Run: bun run scripts/dev-tunnel.ts up'
		);
		return;
	}
	const pid = isRunning();
