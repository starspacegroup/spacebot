 ...
 *
 * Usage:
 *   bun scripts/gateway-latency.ts              # Live test (connect + API pings)
 *   bun scripts/gateway-latency.ts --history     # Show historical data from D1
 *   bun scripts/gateway-latency.ts --history --local           # Query local D1
 *   bun scripts/gateway-latency.ts --history --range 1h        # Custom range (1h, 6h, 24h, 7d, 30d)
 *   bun scripts/gateway-latency.ts --history --recent 20       # Show last N snapshots
 */
...
async function runHistory() {
	...
	const recentLimit =
		recentIdx !== -1 && args[recentIdx + 1] ? parseInt(args[recentIdx + 1], 10) : 10;
	...
	try {
		...
		if (rows.length === 0) {
			...
			console.log(
				`  ${'─'.repeat(10)} ${'─'.repeat(14)} ${'─'.repeat(8)} ${'─'.repeat(12)} ${'─'.repeat(20)}`
			);

			for (const row of rows) {
				const latency =
					row.heartbeat_latency_ms != null ? `${row.heartbeat_latency_ms} ms` : '—';
