...
import { writePersistedRunnerToken } from './token-store';
import {
...
import { readPersistedProviderConfig, writePersistedProviderConfig } from './provider-config';
import {
...
const IS_GHOSTTY =
	/ghostty/i.test(process.env.TERM ?? '') || /ghostty/i.test(process.env.TERM_PROGRAM ?? '');
...
function summarizeRunnerLine(line: string): string | null {
	...
	if (
		line.includes('Authenticated - runner ready') ||
		line.includes('Authenticated — runner ready')
	) {
...
function renderChatMessage(msg: ChatMessageLike): ChatRenderRow[] {
	...
	const headerText = meta ? `${icon} ${ts}  ${author}  ·  ${meta}` : `${icon} ${ts}  ${author}`;
...
function fitSegmentsToWidth(
	...
	for (const segment of segments) {
		...
		const clipped =
			segment.text.length > remaining ? segment.text.slice(0, remaining) : segment.text;
		if (clipped.length > 0) {
...
function createScrollablePanel(
	...
	const scrollSuffix = total > safeHeight ? (offset > 0 ? ` ↑${offset}/${maxOffset}` : ` ▼`) : '';
...
function formatStatus(
	status: 'running' | 'stopped' | 'loading' | 'ready' | 'error',
	tick?: number
): string {
	switch (status) {
		case 'running':
...
interface RunnerStatusFrame {
	state?: RunnerSocketState | 'heartbeat' | 'auth-rejected';
	ts?: number;
	rejection?: string;
}
...
function parseRunnerSocketState(line: string): RunnerSocketState | null {
	...
	if (lower.includes('websocket connected') || lower.includes('authenticated'))
		return 'connected';
	...
	if (
		...
		lower.includes('reconnecting in')
	)
		return 'reconnecting';
...
function App({
	...
	const [permissionConfig, setPermissionConfig] = useState<PersistedPermissionConfig | null>(
	...
	const [mode, setMode] = useState<
		'prompt' | 'permission-setup' | 'provider-setup' | 'dashboard'
	>(() => {
	...
	const [serviceMessage, setServiceMessage] = useState(
		normalizeDisplayText(initialStatus.detail)
	);
	...
	// Guards the automatic browser token negotiation (missing/revoked token) so
	// it fires at most once per session — manual /token retries stay available.
	const autoNegotiateRef = useRef(false);
	...
	// Copilot state
	const [copilotAvailability, setCopilotAvailability] = useState<CopilotAvailability | null>(
		null
	);
	const [activeCopilotConfig, setActiveCopilotConfig] = useState<CopilotConfig | null>(() =>
		getCopilotConfig()
	);
	...
	const [providerSetupStep, setProviderSetupStep] = useState<
		'provider' | 'model' | 'copilot-model'
	>('provider');
	...
	const [permissionSetupConfirm, setPermissionSetupConfirm] = useState<null | 'home' | 'root'>(
		null
	);
	...
	const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
		{
			type: 'system',
			text: 'Welcome to SpaceBot Runner. Type /help for available commands.',
			ts: Date.now(),
			author: displayName,
		},
	]);
	...
	useEffect(() => {
		...
		if (socketState === 'connected') {
			if (disconnectAlert) {
				flashStatus(
					'success',
					'WebSocket reconnected. Runner is receiving jobs again.',
					3000
				);
			}
			setDisconnectAlert(null);
	...
	const autostartOptions: PromptOption[] = [
		{
			...
			title: 'Skip for now',
			description:
				'Continue to the dashboard. You can install autostart later with `i` or /autostart.',
		},
		{
			id: 'install',
			title: `Install ${formatInstallKind(autostartStatus.installKind)} (recommended)`,
			description:
				'Persist the current SPACEBOT_/RUNNER_ environment so the runner starts automatically.',
		},
		{
			id: 'never',
			title: "Don't ask again",
			description:
				'Suppress this prompt until you install autostart manually from the dashboard.',
		},
	];
	...
	function appendLogLines(lines: string[]) {
		...
		for (const line of cleaned) {
			...
			if (frame) {
				...
				if (frame.state === 'auth-rejected') {
					nextSocketState = 'disconnected';
					handleAuthRejection(frame.rejection ?? 'authentication rejected');
					continue;
				}
	...
	/**
	 * The runner child reported that the server refused its token (e.g.
	 * revoked). Retrying with the same token is pointless — stop the child and
	 * negotiate a fresh token in the browser automatically, once per session.
	 */
	function handleAuthRejection(rejection: string) {
		stopRunnerChild();

		if (!rejection.includes('HTTP 401') || autoNegotiateRef.current) {
			setRunnerState(
				clipForUi(
					`Server refused the connection (${rejection}). Use /token to negotiate a new token.`,
					160
				)
			);
			return;
		...
		autoNegotiateRef.current = true;
		addChatMessage({
			type: 'system',
			text: `The server rejected the runner token (${rejection}). Opening your browser to negotiate a fresh one…`,
		});
		void negotiateToken();
	}
	...
	function startRunnerChild(tokenOverride?: string) {
		...
		if (!effectiveToken || !effectiveToken.startsWith('sbr_')) {
			setRunnerState(
				'No runner token configured. Use /token to fetch one from the production site.'
			);
			return;
		}
	...
	async function handleChatCommand(input: string) {
		...
		if (text === '/panels' || text === '/toggle-panels') {
			...
			addChatMessage({
				type: 'system',
				text: `Panels ${!panelsVisible ? 'hidden' : 'shown'}.`,
			});
			return;
		}
		...
		if (text.startsWith('/connect ')) {
			...
			if (activeProvider === 'copilot') {
				...
				if (!c || (!c.copilotCli && !(c.ghCopilot && c.ghAuthed))) {
					addChatMessage({
						type: 'error',
						text: 'Copilot CLI is not available. Run `/providers` for status.',
					});
					return;
				}
			...
			if (!models.some((m) => m.name === model)) {
				addChatMessage({
					type: 'error',
					text: `Model not found: ${model}. Use /models first.`,
				});
				return;
			}
			...
			addChatMessage({
				type: 'success',
				text: `Connected to model: \`${model}\` (provider: ollama)`,
			});
			return;
		}
		...
		if (text === '/providers') {
			...
			lines.push(
				`- **Ollama**: ${ollamaRunning ? `running · ${ollamaModels.length} model(s)` : ollamaInstalled ? 'installed but not running' : 'not detected'}`
			);
			if (copilotAvailability) {
				const c = copilotAvailability;
				const ghPart = c.ghCopilot
					? c.ghAuthed
						? 'ready'
						: 'unauthed (run `gh auth login`)'
					: 'not installed';
				lines.push(
					`- **GitHub Copilot CLI**: ${c.copilotCli ? 'standalone ready' : 'not installed'}`
				);
				lines.push(`- **gh copilot extension**: ${ghPart}`);
			}
			lines.push('');
			lines.push(
				`> Active: \`${activeProvider ?? 'none'}\` · switch with \`/provider ollama\` or \`/provider copilot\``
			);
			addChatMessage({ type: 'system', text: lines.join('\n') });
			return;
		}
		...
		if (text.startsWith('/provider')) {
			...
			if (arg === 'ollama') {
				...
				if (!ollamaRunning) {
					addChatMessage({
						type: 'error',
						text: 'Ollama provider selected, but Ollama is not running. Start it, then `/models` and `/connect <model>`.',
					});
				} else if (!(activeOllamaConfig ?? getOllamaConfig())?.model) {
					addChatMessage({
						type: 'success',
						text: 'Switched to Ollama. Use `/models` then `/connect <model>` to pick a model.',
					});
				} else {
					addChatMessage({
						type: 'success',
						text: `Switched to Ollama (\`${(activeOllamaConfig ?? getOllamaConfig())!.model}\`).`,
					});
				}
				return;
			}
			...
			if (copilot.ghCopilot && !copilot.ghAuthed && !copilot.copilotCli) {
				addChatMessage({
					type: 'error',
					text: '`gh copilot` is installed but `gh` is not authenticated. Run `gh auth login`, then retry.',
				});
				return;
			}
			addChatMessage({
				type: 'success',
				text: `Switched to GitHub Copilot _(${copilot.summary})_.`,
			});
			return;
		}
		...
		if (text.startsWith('/')) {
			addChatMessage({
				type: 'error',
				text: `Unknown command: ${text}. Type /help for help.`,
			});
			return;
		}

		// ---- LLM dispatch ----
		const shouldUseSpaceBotAssistant =
			isSpaceBotManagementRequest(text) && runnerToken.startsWith('sbr_');
		...
		if (activeProvider === 'copilot') {
			...
			const preferVia: 'copilot' | 'gh' = cfg?.via ?? (copilot.copilotCli ? 'copilot' : 'gh');
		...
		if (!config?.model) {
			addChatMessage({
				type: 'error',
				text: 'No model selected. Use `/models` then `/connect <model>`.',
			});
			return;
		}
			...
			if (!runnerToken || !runnerToken.startsWith('sbr_')) {
				setServiceMessage(
					normalizeDisplayText(
						'A runner token is required before installing auto-start. Press k first.'
					)
				);
				if (!activeProvider) {
					setMode('provider-setup');
	...
	const buildPermissionOptions = (): PermissionOption[] => {
		return [
			...
			},
			{
				...
				summary:
					'Allow jobs anywhere under your home directory. The runner can touch your dotfiles and projects.',
				summaryTone: 'warning',
			...
			},
			{
				...
				summary:
					'Allow jobs anywhere on this machine, including /etc and /usr. STRONGLY DISCOURAGED.',
				summaryTone: 'error',
			...
			},
			{
				...
				summary:
					'Continue without setting a scope. The runner will reject every working_dir until you choose one.',
				summaryTone: 'muted',
	...
	function commitPermissionMode(targetMode: PermissionMode) {
		...
		appendLogLines([
			`Permissions set to ${describePermissionMode(targetMode)} → ${paths.join(', ')}`,
		]);
		addChatMessage({
			type: 'success',
	...
	function handlePermissionSetupKey(key: {
		name?: string;
		sequence?: string;
		shift?: boolean;
		ctrl?: boolean;
	...
	function selectProviderFromWizard(id: 'ollama' | 'copilot' | 'skip') {
		...
		if (!c || (!c.copilotCli && !(c.ghCopilot && c.ghAuthed))) {
			setProviderSetupError(
				"Copilot isn't ready yet. Install the CLI or authenticate gh, then retry."
			);
			return;
		}
		setActiveProvider('copilot');
	...
	function selectCopilotModelFromWizard(modelId: string) {
		...
		addChatMessage({
			type: 'success',
			text: `Switched to GitHub Copilot · \`${modelId}\`${costPart} _(${label})_.`,
		});
		setModelPickerFromCommand(false);
		setMode('dashboard');
	}

	function handleProviderSetupKey(key: {
		name?: string;
		sequence?: string;
		shift?: boolean;
		ctrl?: boolean;
		...
		if (providerSetupStep === 'provider') {
			...
			if (key.name === 'return') {
				...
				if (!chosen.selectable) {
					setProviderSetupError(
						chosen.hint ?? "This option isn't ready. Choose another or Skip."
					);
					return;
				}
		...
		if (providerSetupStep === 'model') {
			...
			if (key.name === 'up') {
				setModelSetupSelection(
					(s) => (s + ollamaModels.length - 1) % Math.max(1, ollamaModels.length)
				);
				return;
			}
			if (key.name === 'down' || key.name === 'tab') {
		...
		if (providerSetupStep === 'copilot-model') {
			...
			if (key.name === 'up') {
				setCopilotModelSetupSelection(
					(s) => (s + COPILOT_MODELS.length - 1) % COPILOT_MODELS.length
				);
				return;
			}
			if (key.name === 'down' || key.name === 'tab') {
	...
	async function negotiateToken() {
		...
		const persisted = writePersistedRunnerToken(result.token);
		if (persisted.ok) {
			appendLogLines(['Token stored in the spacebot config dir for future launches.']);
		} else {
			appendLogLines([`Could not persist negotiated token: ${persisted.error}`]);
		}
	...
	// Connect immediately on launch when a token and permission scope are
	// already configured — the autostart/provider prompts must not delay the
	// WebSocket connection. startRunnerChild is a no-op if a child exists, so
	// the dashboard boot below won't double-start. The child is stopped only
	// when the TUI unmounts, not on wizard/dashboard mode transitions.
	useEffect(() => {
		if (runnerToken.startsWith('sbr_')) {
			if (effectiveAllowedPaths.length > 0) {
				startRunnerChild();
			}
		} else if (!autoNegotiateRef.current) {
			// No token at all — negotiate one in the browser right away instead
			// of waiting for the user to discover /token.
			autoNegotiateRef.current = true;
			addChatMessage({
				type: 'system',
				text: 'No runner token configured. Opening your browser to negotiate one from the production site…',
			});
			void negotiateToken();
		}
		return () => {
	...
	useKeyboard((key) => {
		...
		if (mode === 'prompt') {
			if (key.name === 'up') {
				setPromptSelection(
					(current) => (current + promptOptions.length - 1) % promptOptions.length
				);
				return;
			}
		...
		if (chatMode) {
			...
			if (key.name === 'tab' && chatInput.startsWith('/')) {
				...
				const matches = SLASH_COMMANDS.filter(
					(c) => c.cmd.startsWith(partial) && c.cmd !== partial
				);
				if (matches.length === 1) {
	...
	const heartbeatAgeMs =
		lastSocketHeartbeatAtRef.current > 0
			...
			: null;
	const heartbeatAgeText = heartbeatAgeMs === null ? 'n/a' : formatHeartbeatAgeMs(heartbeatAgeMs);
	const heartbeatAgeTone: keyof typeof THEME =
		heartbeatAgeMs === null ? 'muted' : getHeartbeatHealthTone(heartbeatAgeMs);
	...
	const buildAgentStrip = (): StyledLine => {
		...
		const hbTone: keyof typeof THEME =
		...
		const serviceKindCompact =
			autostartStatus.installKind === 'systemd-user'
				? 'systemd'
		...
		if (activeProvider === 'ollama') {
			...
			badges.push({
				...
				text: activeOllamaConfig?.model
					? `🤖 ${activeOllamaConfig.model}`
					: '🤖 Ollama (no model)',
			});
			...
			badges.push({
				...
				text: ready
					? `🦑 auto ${autoCompact} · ${m}`
					: `🦑 auto ${autoCompact} · Copilot setup`,
			});
		} else {
	...
	const buildTabHint = (): string => {
		...
		const matches = SLASH_COMMANDS.filter(
			(c) => c.cmd.startsWith(chatInput) && c.cmd !== chatInput
		);
		if (matches.length === 1) {
	...
	const buildScreenLines = (): StyledLine[] => {
		...
		const logoRows = [' ╭──────╮ ', ` │ ${leftEye}██${rightEye} │ `, ' ╰──────╯ '];
		...
		const forceCompactDashboard =
			mode === 'dashboard' && renderHeight < MIN_HEIGHT_FOR_FULL_DASHBOARD;

		if (mode === 'prompt') {
			const promptRows: Array<{ tone: keyof typeof THEME; text: string }> = [
				{
					tone: 'info',
					text: 'Autostart is recommended but optional — you can install it later from the dashboard.',
				},
		...
		if (mode === 'permission-setup') {
			...
			if (permissionSetupConfirm) {
				...
				if (isRoot) {
					...
					rows.push({
						tone: 'error',
						text: '⚠️  ROOT FILESYSTEM ACCESS — VERY HIGH RISK ⚠️',
					});
					rows.push({
						tone: 'error',
						text: '════════════════════════════════════════════════════════════',
					...
					rows.push({
						tone: 'warning',
						text: 'You are about to grant the runner permission to execute commands',
					});
					rows.push({
						tone: 'warning',
						text: 'ANYWHERE on this machine, including /etc, /usr, /var, and so on.',
					});
					rows.push({ tone: 'muted', text: '' });
					rows.push({
						tone: 'error',
						text: '  • A compromised SpaceBot account can delete or alter system files.',
					});
					rows.push({
						tone: 'error',
						text: '  • A buggy prompt could damage your OS install irrecoverably.',
					});
					rows.push({
						tone: 'error',
						text: '  • Secrets in /etc, /root, browser profiles, etc. are all reachable.',
					});
					rows.push({ tone: 'muted', text: '' });
					rows.push({
						tone: 'warning',
						text: 'Only continue on machines you fully own and are willing to wipe.',
					});
				} else {
					...
					rows.push({
						tone: 'warning',
						text: `The runner will be allowed to read and write anywhere under ${resolvePathsForMode('home')[0]}.`,
					});
					rows.push({ tone: 'muted', text: '' });
					rows.push({
						tone: 'info',
						text: '  • This includes your projects, dotfiles, SSH keys, browser profiles, and credentials.',
					});
					rows.push({
						tone: 'info',
						text: '  • Safer alternative: the Sandbox (~/spacebot) scope.',
					});
				}
				rows.push({ tone: 'muted', text: '' });
				...
				const lines: StyledLine[] = [
					...headerLines,
					...createPanel(
						isRoot ? 'CONFIRM · ROOT ACCESS' : 'CONFIRM · HOME ACCESS',
						rows,
						renderWidth
					),
				];
			...
			const rows: Array<{ tone: keyof typeof THEME; text: string }> = [
				...
				{
					tone: 'muted',
					text: '↑/↓ or Tab to move · Enter to choose · Esc to skip for now',
				},
				{ tone: 'muted', text: '' },
			];
		...
		if (mode === 'provider-setup') {
			...
			if (providerSetupStep === 'model') {
				const modelRows: Array<{ tone: keyof typeof THEME; text: string }> = [
					...
					{
						tone: 'muted',
						text: '↑/↓ to move · Enter to connect · Esc/Shift+Tab to go back',
					},
					{ tone: 'muted', text: '' },
				];
				if (ollamaModels.length === 0) {
					modelRows.push({
						tone: 'warning',
						text: 'No models available. Pull one with `ollama pull <name>`.',
					});
				} else {
			...
			const cmRows: Array<{ tone: keyof typeof THEME; text: string }> = [
				...
				{
					tone: 'muted',
					text: 'Cost shown as the GitHub premium-request multiplier per prompt.',
				},
				{ tone: 'muted', text: '' },
			];
		...
		if (mode === 'dashboard' && (!panelsVisible || forceCompactDashboard)) {
			...
			const truncatedStatusCompact =
				statusCompact.length > inputInnerCompact
					? statusCompact.slice(0, Math.max(0, inputInnerCompact - 1)) + '…'
					: statusCompact;
			const promptCompact = `${chatMode ? '❯' : '·'} `;
			const cursorCompact = chatMode ? '▏' : '';
			const inputBudgetCompact = Math.max(
				1,
				inputInnerCompact - promptCompact.length - cursorCompact.length
			);
			const visibleInputCompact =
				chatInput.length > inputBudgetCompact
			...
			const hintBudgetCompact = Math.max(
				0,
				inputInnerCompact -
					(promptCompact.length + visibleInputCompact.length + cursorCompact.length)
			);
			const visibleTabHintCompact =
				hintBudgetCompact > 0
		...
		const statusRows: Array<{ tone: keyof typeof THEME; text: string }> = [
			{
				tone: childRunning ? 'success' : 'warning',
				text: `Session: ${formatStatus(childRunning ? 'running' : 'stopped', spinnerTick)}`,
			},
			{
				tone:
					socketState === 'connected'
			...
			{
				tone: tokenNegotiating ? 'info' : 'muted',
				text: tokenNegotiating
					? `${getSpinner(spinnerTick)} Negotiating token via browser...`
					: 'Token negotiation idle',
			},
		];
		...
		const configRows: Array<{ tone: keyof typeof THEME; text: string }> = [
			...
			{ tone: 'value', text: buildKeyValueLine('Host', hostname) },
			{
				...
				text: buildKeyValueLine(
					'Token',
					runnerToken.startsWith('sbr_')
						? `${runnerToken.slice(0, 8)}... configured`
						: 'missing, use /token'
				),
			},
			{
			...
			...(manualOpenUrl
				? [
						{
							tone: 'warning' as keyof typeof THEME,
							text: 'Manual browser fallback required:',
						},
		...
		const truncatedStatus =
			statusText.length > inputInner
				? statusText.slice(0, Math.max(0, inputInner - 1)) + '…'
				: statusText;
		...
		const visibleInput =
			chatInput.length > inputBudget
				? '…' + chatInput.slice(chatInput.length - (inputBudget - 1))
				: chatInput;
		const tabHint = buildTabHint();
		const tabHintDecorated = tabHint ? `  ⇥ ${tabHint}` : '';
		const hintBudget = Math.max(
			0,
			inputInner - (prompt.length + visibleInput.length + cursor.length)
		);
		const visibleTabHint =
			hintBudget > 0
