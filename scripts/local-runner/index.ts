...
function parseBridgePortRange(raw: string | undefined): [number, number] {
	...
	const a = Number.isFinite(aRaw)
		? Math.max(1024, Math.min(65535, Math.trunc(aRaw)))
		: fallback[0];
...
function getBridgeScanUrls(): string[] {
	...
	const single = String(process.env.RUNNER_VSCODE_BRIDGE_URL || '')
		.trim()
		.replace(/\/$/, '');
...
function pickBridgeInstance(
	instances: Array<Record<string, unknown>>,
	payload: Record<string, unknown>
) {
	const requestedUrl =
		typeof payload.bridgeUrl === 'string' ? payload.bridgeUrl.trim().replace(/\/$/, '') : '';
	if (requestedUrl) {
...
function emitStatusFrame(
	...
	try {
		console.log(
			`${STATUS_FRAME_PREFIX} ${JSON.stringify({ state, ts: Date.now(), ...details })}`
		);
	} catch {
	...
	keepaliveTimer = setInterval(
		() => {
			...
			if (
				...
				Date.now() - lastServerMessageAt > DEAD_SOCKET_THRESHOLD_MS
			) {
...
function buildProviderMetadata() {
	...
	const copilotCfg =
		getCopilotConfig() ??
		(persisted.copilot ? { model: persisted.copilot.model, via: persisted.copilot.via } : null);
	...
	const ollamaCfg =
		...
		(persisted.ollama
			? {
					host: persisted.ollama.host,
					port: persisted.ollama.port,
...
function resolveProviderChain(payload: Record<string, unknown>): ProviderChainEntry[] {
	...
	const preferred =
		...
		process.env.SPACEBOT_LLM_PROVIDER === 'ollama'
			...
			: persisted.provider === 'copilot' || persisted.provider === 'ollama'
				? persisted.provider
				: null;
	...
	const addCopilot = () => {
		...
		defaults.push({
			provider: 'copilot',
			model: copilotCfg.model || DEFAULT_COPILOT_MODEL,
			via: copilotCfg.via,
		});
	};
	const addOllama = () => {
...
async function runPromptThroughProviderChain(
	prompt: string,
	payload: Record<string, unknown>
): Promise<{
	ok: boolean;
...
function encodeArtifact(
	buffer: Buffer,
	options: { width?: number; height?: number; captureSource?: string; captureIndex?: number }
): PreparedArtifact {
	return {
...
function captureScreenshotBuffer(): Buffer {
	...
	if (process.platform === 'darwin') {
		const result = spawnSync('screencapture', ['-x', tempPath], {
			encoding: 'utf8',
		...
		const result = spawnSync(SHELL, [SHELL_FLAG, command, '--', tempPath], {
			encoding: 'utf8',
			timeout: SCREENSHOT_TIMEOUT_MS,
		});
		if (result.signal === 'SIGTERM' || result.error?.message?.includes('ETIMEDOUT')) {
...
function captureMacPerDisplayArtifacts(
	...
	try {
		...
		for (let i = 1; i <= displayCount; i++) {
			...
			const result = spawnSync('screencapture', ['-x', '-D', String(i), tempPath], {
				encoding: 'utf8',
				timeout: 30_000,
			});
...
function captureLinuxPerDisplayArtifacts(
	displayItems: Array<Record<string, unknown>>
): PreparedArtifact[] {
	...
	try {
		for (let i = 0; i < outputs.length; i++) {
			...
			const tempPath = join(tempDir, `screen-${i}.png`);
			const result = spawnSync('grim', ['-o', outputName, tempPath], {
				encoding: 'utf8',
				timeout: 30_000,
			});
...
function captureScreenshotArtifacts(
	mode: string,
	displayItems: Array<Record<string, unknown>>
): PreparedArtifact[] {
	if (mode === 'per_display') {
	...
	if (single.length > MAX_ARTIFACT_BYTES) {
		throw new Error(
			`Screenshot exceeds RUNNER_MAX_ARTIFACT_BYTES (${single.length} > ${MAX_ARTIFACT_BYTES}).`
		);
	}

	return [
		encodeArtifact(single, {
			captureSource: mode,
			captureIndex: 0,
		}),
	];
...
async function executeDmJob(payload: Record<string, unknown>): Promise<JobResult> {
	...
	const assistantResult = await callRunnerAssistant(API_URL, TOKEN, {
		...
		selectedGuild:
			payload.selectedGuild && typeof payload.selectedGuild === 'object'
				? (payload.selectedGuild as Record<string, unknown>)
				: null,
		selectedGuildId:
			typeof payload.selectedGuildId === 'string' ? payload.selectedGuildId : null,
	});
		...
		const failureSummary =
			run.attempts.length > 0
				? run.attempts
						.map(
							(entry) =>
								`${entry.provider}/${entry.model}: ${entry.error || 'failed'}`
						)
						.join('; ')
...
async function executeTypedJob(job: Job): Promise<JobResult | null> {
	...
	if (jobType === 'screenshot_capture') {
		...
		try {
			...
			const artifacts = captureScreenshotArtifacts(
				captureMode,
				profile.displays.items as Array<Record<string, unknown>>
			);
			...
			const perDisplayFallback =
				captureMode === 'per_display' &&
				artifacts.length === 1 &&
				profile.displays.count > 1;
			...
			return {
				...
				result: {
					...
					fallbackReason: perDisplayFallback
						? 'per_display_capture_fell_back_to_single_image'
						: null,
				},
			};
		} catch (e) {
	...
	if (jobType === 'vscode_send_copilot_message') {
		...
		const responseText =
			typeof sent.body?.response === 'string' ? sent.body.response.trim() : '';
...
async function drainQueue(ws: WebSocket) {
	...
	while (jobQueue.length > 0) {
		...
		try {
			...
			result = {
				status: 'failed',
				output: `Unexpected runner error: ${e}`,
				exitCode: 1,
				truncated: false,
			};
		}
		...
		journal(
			`job #${job.id}${job.label ? ` (${job.label})` : ''} ${result.status} (type ${job.job_type || 'shell_command'}, exit ${result.exitCode})`
		);
		...
		try {
			...
			sendRunnerEvent(
				'runner.error',
				`Failed to send result for job #${job.id}`,
				undefined,
				'error',
				job.id
			);
...
function connect() {
	...
	ws.onmessage = (event: MessageEvent) => {
		...
		switch (msg.type) {
			case 'connected':
				...
				emitStatusFrame('connected', {
					phase: 'authenticated',
					instanceName: msg.instanceName ?? DISPLAY_NAME,
				});
				journal(`connected to ${API_URL} as ${msg.instanceName ?? DISPLAY_NAME}`);
				sendRunnerEvent('runner.ready', `Runner ready on ${DISPLAY_NAME}`, {
	...
	ws.onerror = (event: Event) => {
		...
		sendRunnerEvent(
			'runner.error',
			`WebSocket error: ${(event as ErrorEvent).message ?? event.type}`,
			undefined,
			'error'
		);
...
async function ensureRunnerHome(): Promise<void> {
	...
	if (!root) {
		if (FORCE_HEADLESS) {
			...
			log(
				`No runner home configured — defaulting to ${root} (set SPACEBOT_RUNNER_HOME or run interactively to change).`
			);
		} else {
	...
	if (scaffold.created.includes(root)) {
		log(
			`SpaceBot home created at ${root} — see ${join(root, 'README.md')} for how to interact via markdown.`
		);
	}
	...
	journal(
		`runner v${RUNNER_VERSION} started (pid ${process.pid}, ${FORCE_HEADLESS ? 'headless' : 'tui'})`
	);
}
...
async function respondToInboxMessage(conversation: string): Promise<string | null> {
	...
	const prompt = [
		...
		'',
		memoryContext
			? `What you know about this machine and stored memories:\n${memoryContext}\n`
			: '',
		'The note:',
		conversation,
		'',
...
function startHeadlessRunner() {
	...
	if (ALLOWED_PATHS.length > 0) {
		...
		if (PERMISSION_MODE === 'root') {
			warn(
				'⚠️  PERMISSION MODE = ROOT — the runner can execute commands anywhere on this machine.'
			);
			warn(
				'⚠️  This is the highest-risk setting. Only use it on dedicated machines you control.'
			);
		} else if (PERMISSION_MODE === 'home') {
			warn(
				'⚠️  PERMISSION MODE = HOME — the runner can read/write anywhere under your home directory.'
			);
		}
	} else {
		warn('⚠️  No allowed-paths scope is configured — ALL jobs will be rejected.');
		warn(
			'⚠️  Launch the runner interactively and pick a scope (sandbox/home/root), or set RUNNER_ALLOWED_PATHS.'
		);
	}
	log('');
...
async function main() {
	if (process.argv.includes('--version')) {
		console.log(
			`spacebot-runner ${RUNNER_VERSION} (${process.platform}/${process.arch}${IS_COMPILED ? ', compiled' : ''})`
		);
		return;
	}
