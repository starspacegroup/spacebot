...
/**
 * Expand a leading ~ to the user's home directory. Values from .env files
 * (e.g. RUNNER_ALLOWED_PATHS=~/) are never shell-expanded, and resolve("~/")
 * would otherwise yield a literal "<cwd>/~" path that matches nothing.
 */
...
export function readPersistedPermissionConfig(): PersistedPermissionConfig | null {
	try {
		...
		if (mode !== 'sandbox' && mode !== 'home' && mode !== 'root' && mode !== 'custom')
			return null;
...
export function writePersistedPermissionConfig(config: PersistedPermissionConfig): {
	ok: boolean;
	error?: string;
} {
	try {
 ...
 */
export function resolveEffectiveAllowedPaths(): {
	paths: string[];
	...
	if (process.env.RUNNER_ALLOWED_PATHS) {
		return {
			paths: process.env.RUNNER_ALLOWED_PATHS.split(':')
				.filter((p) => p.length > 0)
