...
const AUTO_BEGIN = "<!-- spacebot:auto:begin — this block is rewritten on every runner start; edit outside it -->";
...
function renderAutoSection(profile: RunnerSystemProfile): string {
  const tools = Object.entries(profile.installedApps.tools).sort(([a], [b]) => a.localeCompare(b));
  ...
  const lines = [
    ...
    profile.displays.count > 0
      ? profile.displays.items.map((d) => `- ${d.id}: ${d.width}x${d.height} at ${d.x},${d.y}${d.primary ? " (primary)" : ""}`).join("\n")
      : "- None detected",
    "",
    "## Package managers",
    "",
...
function diffProfiles(previous: RunnerSystemProfile | null, next: RunnerSystemProfile): string[] {
  ...
  if ((previous.displays?.count ?? 0) !== next.displays.count) {
    changes.push(`Display count changed: ${previous.displays?.count ?? 0} → ${next.displays.count}`);
  }
  ...
  if ((previous.hardware?.totalMemoryBytes ?? 0) !== next.hardware.totalMemoryBytes) {
    changes.push(`Total memory changed: ${formatGiB(previous.hardware?.totalMemoryBytes ?? 0)} → ${formatGiB(next.hardware.totalMemoryBytes)}`);
  }
  ...
  try {
    ...
    if (!existsSync(systemMd)) {
      ...
      if (beginIdx !== -1 && endIdx !== -1 && endIdx > beginIdx) {
        content = existing.slice(0, beginIdx) + autoSection + existing.slice(endIdx + AUTO_END.length);
      } else {
...
export function saveMemory(root: string, title: string, body: string): { ok: boolean; file?: string; error?: string } {
  try {
    ...
    writeFileSync(file, [`# ${title.trim()}`, "", `_Saved: ${savedAt}_`, "", body.trim(), ""].join("\n"), "utf8");

    const indexFile = join(root, "MEMORY.md");
    const header = "# SpaceBot Memory Index\n\nOne line per stored memory. Full content lives in `memory/`.\n\n";
  ...
  try {
    ...
    if (existsSync(cacheFile)) {
      ...
      const tools = Object.entries(profile.installedApps?.tools ?? {})
        ...
        .map(([name]) => name);
      parts.push(
        `This machine: ${profile.os.hostname} (${profile.os.platform}/${profile.os.arch}), `
