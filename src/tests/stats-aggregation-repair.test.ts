...
import { runDailyStatsRepair, runHourlyAggregation } from '../lib/server/cron-jobs.js';
		...
		return {
			...
			async all() {
				if (
					...
					sql.includes("event_type IN ('VOICE_JOIN'")
				) {
					return { results: [] }; // processVoiceSessions: nothing unprocessed
				}
				if (
					sql.includes('FROM aggregated_stats') &&
					sql.includes("period_type = 'hourly'")
				) {
				...
				if (sql.includes('distinct_guild')) {
					return { results: [{ guild_id: 'guild1' }] }; // getGuildsWithLogs
				}
				return { results: [] };
			},
			async run() {
...
describe('runHourlyAggregation', () => {
	it('is incremental by default so the hourly tick does not re-walk the repair window', async () => {
		const checkpoint = sqlDateTime(new Date(Date.now() - 2 * 60 * 60 * 1000));
		const db = new FakeAggDb(checkpoint);

		const result = await runHourlyAggregation(db);

		expect(result.guildsProcessed).toBe(1);
		expect(result.totalHourlyPeriods).toBeLessThanOrEqual(3);
	});

	it('re-walks the full repair window when the daily repair pass asks for it', async () => {
		const checkpoint = sqlDateTime(new Date(Date.now() - 2 * 60 * 60 * 1000));
		const db = new FakeAggDb(checkpoint);

		const result = await runDailyStatsRepair(db);

		expect(result.guildsProcessed).toBe(1);
		expect(result.totalHourlyPeriods).toBeGreaterThan(150);
	});
});
