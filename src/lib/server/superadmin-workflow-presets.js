...
export const OPERATION_TEMPLATES = [
  ...
  },
  {
    ...
    description: "Example workflow that talks to a local runner: collects a system profile and runs a shell command, waiting for results. Set runner_token_id on both task nodes before enabling.",
    category: "runners",
    execution_backend: "cloudflare_workflows",
    ...
    canvas_json: {
      nodes: [
        { id: "start", type: "trigger", title: "Manual Trigger", position: { x: 0, y: 0 }, data: { source: "operator" } },
        { id: "profile", type: "task", title: "Collect System Profile", position: { x: 0, y: 140 }, data: { operation: "local_runner_job", runner_token_id: 0, job_type: "system_profile", wait_for_result: true, wait_timeout_seconds: 60, queue_key: "ops.runners.profile" } },
        { id: "echo", type: "task", title: "Run Health Command", position: { x: 0, y: 280 }, data: { operation: "local_runner_job", runner_token_id: 0, job_type: "shell_command", command: "uptime && df -h / | tail -1", wait_for_result: true, wait_timeout_seconds: 60, queue_key: "ops.runners.health" } },
      ],
      edges: [
 ...
 * list of created slugs. Used by the dispatcher for first-run seeding and by
 * the UI's "install presets" path.
 */
export async function seedSuperadminWorkflowPresets(db, userId = "system") {
  const existing = await listSuperadminWorkflowTemplates(db, { includeArchived: true, limit: 200 });
  const existingSlugs = new Set(existing.map((t) => t.slug));
  const created = [];

  for (const preset of OPERATION_TEMPLATES) {
    if (existingSlugs.has(preset.slug)) continue;
    const result = await createSuperadminWorkflowTemplate(db, userId, preset);
    if (result.success) {
