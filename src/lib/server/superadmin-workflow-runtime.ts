 ...
 */
function resolveDataDeep(value, runtime, depth = 0) {
...
async function runCustomActions(actions, stage, runtime, options: { swallowErrors?: boolean } = {}) {
...
export async function executeWorkflowGraphRuntime({ db, platform, fetch, template, run, inputJson }) {
      ...
      if (currentNode.type === "trigger") {
        ...
        if (operationKey) {
          try {
            ...
            // let the step fail.
            await runCustomActions(moduleConfig.custom_actions, "on_failure", runtime, { swallowErrors: true });
...
export async function executeLegacyBridgeRun({ db, platform, fetch, template, run }) {
  ...
  const secret = platform?.env?.CRON_SECRET
    ...
    || (typeof process !== "undefined" ? (process.env?.CRON_SECRET || process.env?.INTERNAL_API_KEY) : undefined);
 ...
 */
export async function executeSuperadminWorkflowRun({ db, platform, fetch, template, run, inputJson }) {
  const nodes = template?.canvas_json?.nodes || [];
