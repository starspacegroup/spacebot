...
async function runLocalRunnerJob(ctx) {
	...
	const token = await db
		.prepare('SELECT id, user_id FROM local_runner_tokens WHERE id = ? AND revoked = 0')
		.bind(tokenId)
		.first();
	...
	const created = await createRunnerJob(db, token.user_id, tokenId, {
		...
		job_type:
			typeof data.job_type === 'string' && data.job_type.trim()
				? data.job_type.trim()
				: 'shell_command',
		payload_json: payload,
		working_dir: data.working_dir ?? null,
		label:
			data.label || `workflow:${ctx.template?.slug || ctx.template?.id} run#${ctx.run?.id}`,
	...
	while (
		row &&
		!['completed', 'failed', 'canceled'].includes(String(row.status)) &&
		Date.now() < deadline
	) {
	...
	if (finalStatus === 'failed' || finalStatus === 'canceled') {
		...
		throw Object.assign(new Error(`Runner job #${jobId} ${finalStatus}${detail}`), {
			operationResult: result,
		});
	}
	if (!finished && data.fail_on_timeout === true) {
		throw Object.assign(
			new Error(`Runner job #${jobId} did not finish within ${waitSeconds}s`),
			{ operationResult: result }
		);
	}
...
async function runHttpRequest(ctx) {
	...
	if (!/^https?:\/\//i.test(url) && !url.startsWith('/')) {
		throw new Error(
			'http_request requires data.url (absolute http(s) URL or app-relative path)'
		);
	}
...
async function runLegacyCronJob(ctx) {
	...
	if (!response.ok) {
		throw Object.assign(
			new Error(
				`Legacy cron job "${jobName}" failed: ${bodyJson.error || `HTTP ${response.status}`}`
			),
...
const OPERATIONS = {
	...
	cleanupOldData: {
		description: 'Prune old stats, expired operational data, and aged-out raw event logs',
		handler: async (ctx) => ({
			prunedStats: await pruneOldStats(ctx.db),
			cleanup: await cleanupOldData(ctx.db),
			// Added here rather than as a new preset node on purpose: the preset
			// seeder only tops up MISSING slugs, so a definition change would not
			// reach an already-seeded production template without an operator
...
export function listWorkflowOperations() {
	return Object.entries(OPERATIONS).map(([key, def]) => ({
		...
		destructive: (def as any).destructive === true,
	}));
}
...
export async function executeWorkflowOperation(operationKey, ctx) {
	...
	if (!definition) {
		throw new Error(
			`Unknown workflow operation: "${key}". Registered operations: ${Object.keys(OPERATIONS).join(', ')}`
		);
	}
