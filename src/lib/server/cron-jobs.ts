 ...
 *
 * The plain `SELECT DISTINCT guild_id FROM event_logs WHERE created_at >= ?`
 * form has to walk every event row in the window — event_logs is the largest
 * table in the database, so that was tens of millions of D1 rows-read per day
 * for a result that is only ever a handful of guild ids.
 *
 * Instead: a recursive "loose index scan" hops between distinct guild_ids
 * using idx_event_logs_guild_created (one seek per guild), then an EXISTS
 * check per guild answers the recency question with one more seek. Cost is
 * O(number of guilds) rather than O(number of events).
 */
export async function getGuildsWithLogs(db, since = '-7 days') {
	if (!db) return [];
 ...
 * recent logs.
 *
 * Runs incrementally (`repair: false`) — only the periods after the stored
 * checkpoint. The repair sweep re-walks a 7-day hourly / 14-day daily window
 * regardless of the checkpoint, which is the right thing to do occasionally
 * but was previously happening on *every* hourly tick: ~168 hourly periods
 * re-aggregated per guild per run, each costing several queries over
 * event_logs and voice_sessions. `runDailyStatsRepair` keeps the self-healing
 * behaviour on a once-a-day cadence instead.
 */
export async function runHourlyAggregation(db, options: Record<string, any> = {}) {
	...
	for (const guildId of guilds) {
		try {
			const { hourly, daily } = await runStatsAggregation(db, guildId, { repair });
...
/**
 * Re-walk the full repair window (7 days hourly / 14 days daily) for every
 * guild with recent logs, healing periods that were aggregated before all of
 * their events had landed.
 *
 * This is the expensive variant of `runHourlyAggregation`, so it belongs on a
 * daily cadence rather than an hourly one.
 */
export async function runDailyStatsRepair(db) {
	return runHourlyAggregation(db, { repair: true });
}
...
export async function runDailyRefresh(db, botToken) {
	...
	if (guilds.length === 0) {
		return {
			stats: { processed: 0, failed: 0 },
			cache: { processed: 0, failed: 0, skipped: 0 },
		};
	}
		...
		try {
			...
			try {
				...
				if (cacheResult.success) {
					...
					cacheResult.error?.includes('Intent') ||
					cacheResult.error?.includes('permission')
				) {
					// Server Members Intent not enabled or missing permissions - skip but don't count as failure
					results.cache.skipped++;
					log.debug(
						`[CronJobs] Cache refresh skipped for ${guild.id}: ${cacheResult.error}`
					);
				} else {
					results.cache.failed++;
					log.warn(
						`[CronJobs] Cache refresh failed for ${guild.id}: ${cacheResult.error}`
					);
				}
			} catch (cacheError) {
				results.cache.failed++;
	...
	// Heal any aggregated periods that were built before all of their events
	// landed. The hourly job runs incrementally, so this is the pass that
	// re-walks the repair window.
	try {
		results.statsRepair = await runDailyStatsRepair(db);
	} catch (repairError) {
		log.error('[CronJobs] Daily stats repair failed:', repairError);
	}
	...
	log.info(
		`[CronJobs] Daily refresh complete: ${results.stats.processed} stats, ${metadataResults.processed} metadata, ${results.cache.processed} caches (${results.cache.totalMembers} members, ${results.cache.totalRoles} roles)`
	);
...
export async function runCacheRefresh(db, botToken) {
	...
	log.info(
		`[CronJobs] Cache refresh complete: ${results.processed} guilds, ${results.totalMembers} members, ${results.totalRoles} roles`
	);
...
export async function deleteAggregatedStats(db) {
  ...
  `
	...
	log.info(
		`[CronJobs] Deleted ${results.deletedHourly} hourly and ${results.deletedDaily} daily aggregated stats`
	);
	return results;
}
...
export async function runRebuildStats(db) {
	...
	try {
		...
		const guilds = await getGuildsWithLogs(db, '-90 days');
		...
		log.info(
			`[CronJobs] Rebuild complete: ${results.guildsProcessed} guilds, ${results.totalHourlyPeriods} hourly, ${results.totalDailyPeriods} daily periods`
		);
	} catch (error) {
