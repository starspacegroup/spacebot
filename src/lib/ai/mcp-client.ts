...
export class MCPClient {
	...
	constructor(options: MCPClientOptions = {}) {
		...
		log.debug(
			`[MCP] Client initialized - accountId: ${this.accountId ? 'SET' : 'MISSING'}, apiToken: ${this.apiToken ? 'SET' : 'MISSING'}, useLocalDb: ${this.useLocalDb}`
		);
	}
	...
	isConfigured() {
		...
		if (!configured) {
			log.warn(
				`[MCP] Client not configured - accountId: ${this.accountId ? 'SET' : 'MISSING'}, apiToken: ${this.apiToken ? 'SET' : 'MISSING'}`
			);
		}
		return configured;
	}
	...
	async initLocalDb() {
		...
		try {
			...
			const files = readdirSync(wranglerDbPath).filter(
			...
			if (!selectedFile) {
				...
				log.warn(
					`[MCP] Could not find database with automations table, falling back to: ${selectedFile}`
				);
			}
	...
	async executeD1Query(sql, params = []) {
		...
		if (!this.isConfigured()) {
			throw new Error(
				'MCP Client not configured: Missing CLOUDFLARE_ACCOUNT_ID or CLOUDFLARE_API_TOKEN'
			);
		}
	...
	async startLocalRunnerTask(userId, options: any = {}) {
		...
		const boundedTimeout = Math.max(
			30,
			Math.min(3600, Math.trunc(Number(timeoutSeconds) || 300))
		);
		...
		for (const rawName of instanceNames || []) {
			const query = String(rawName || '')
				.trim()
				.toLowerCase();
			if (!query) continue;
			const matches = runners.filter(
				(runner) =>
					String(runner.display_name || '')
						.toLowerCase()
						.includes(query) ||
					String(runner.hostname || '')
						.toLowerCase()
						.includes(query) ||
					String(runner.token_name || '')
						.toLowerCase()
						.includes(query)
			);
		...
		// Synchronously await the result when the task went to exactly one online
		// runner, so the assistant can report real output/exit code in the same
		// turn instead of only "queued #N". Default: wait for a single online
		// target unless the caller explicitly opts out. Multi-target and offline
		// fan-outs stay fire-and-queue (their results arrive later over WS).
		const onlyJob = queuedJobs.length === 1 ? queuedJobs[0] : null;
		const shouldWait =
			waitForResult === true ||
			(waitForResult === undefined &&
				onlyJob !== null &&
				onlyJob.isOnline === true &&
				Boolean(onlyJob.jobId));

		if (shouldWait && onlyJob?.jobId) {
			const boundedWait = Math.max(1, Math.min(90, Math.trunc(Number(waitSeconds) || 25)));
			const waitResult = await this.waitForLocalRunnerJob(userId, onlyJob.jobId, boundedWait);
			onlyJob.waitStatus = waitResult.status; // 'done' | 'timeout' | 'missing'
			if (waitResult.job) {
				onlyJob.status = waitResult.job.status;
				onlyJob.exitCode = waitResult.job.exit_code ?? null;
				onlyJob.output =
					typeof waitResult.job.output === 'string' ? waitResult.job.output : null;
				onlyJob.terminalError = waitResult.job.terminal_error ?? null;
			}
		}
		...
		return {
			...
			queuedJobs,
			unresolvedNames,
			awaited: Boolean(shouldWait && onlyJob?.jobId),
		};
	}
	...
	async cancelLocalRunnerJob(userId, jobId, reason = null) {
       ...
       WHERE id = ?
		...
		if (!changed) {
			return {
				success: false,
				error: 'Job not found or cannot be canceled (only pending or running jobs can be canceled).',
			};
		}
	...
	async retryLocalRunnerJob(userId, jobId) {
       ...
       WHERE id = ?
		...
		if (!changed) {
			return {
				success: false,
				error: 'Job not found or cannot be retried (only failed or canceled jobs can be retried).',
			};
		}
	...
	async resolveLocalRunnerTarget(userId, options: any = {}) {
		...
		if (requireOnline && !selected.is_online) {
			throw new Error(
				`Selected runner ${selected.display_name || selected.hostname || selected.id} is offline`
			);
		}
	...
	async waitForLocalRunnerJob(userId, jobId, maxWaitSeconds = 25) {
		const maxWaitMs =
			Math.max(1, Math.min(90, Math.trunc(Number(maxWaitSeconds) || 25))) * 1000;
	...
	async getLogStats(guildId) {
		const [totalResult, categoryResult, recentResult] = await Promise.all([
			this.executeD1Query('SELECT COUNT(*) as total FROM event_logs WHERE guild_id = ?', [
				guildId,
			]),
			this.executeD1Query(
	...
	async getAutomations(guildId, options: any = {}) {
		...
		return result.results.map((automation) => ({
			...
			trigger_events: automation.trigger_events
				? JSON.parse(automation.trigger_events)
				: automation.trigger_event
					? [automation.trigger_event]
					: [],
			trigger_filters: automation.trigger_filters
				? JSON.parse(automation.trigger_filters)
				: {},
	...
	async getGuildSettings(guildId) {
		...
		return {
			...
			excluded_channels: settings.excluded_channels
				? JSON.parse(settings.excluded_channels)
				: [],
	...
	async getMembersWithRole(guildId, roleNameOrId, limit = 100, db = null) {
		...
		if (db) {
			try {
				const { getMembersWithRoleFromCache, getCacheMetadata } =
					await import('../db/guild-cache.js');
				...
				if (metadata?.members_last_refreshed) {
					const cacheAge =
						Date.now() - new Date(metadata.members_last_refreshed).getTime();
					const maxAge = 2 * 60 * 60 * 1000; // 2 hours

					if (cacheAge < maxAge) {
						log.debug(
							`[MCP] Using cached member data for guild ${guildId} (age: ${Math.round(cacheAge / 60000)}min)`
						);
						const cacheResult = await getMembersWithRoleFromCache(
							db,
							guildId,
							roleNameOrId,
							limit
		...
		if (!targetRole) {
			...
			targetRole =
				guildRoles.find((r) => r.name.toLowerCase() === searchName) ||
		...
		while (allMembers.length < maxMembersToFetch) {
			...
			if (!membersResponse.ok) {
				if (membersResponse.status === 403) {
					throw new Error(
						"Bot doesn't have permission to view members. The 'Server Members Intent' must be enabled in the Discord Developer Portal."
					);
				}
		...
		log.info(
			`[MCP] Found ${membersWithRole.length} members with role "${targetRole.name}" (checked ${allMembers.length} total members)`
		);
	...
	async getTextChannels(guildId, nameFilter = null) {
		...
		if (nameFilter) {
			...
			textChannels = textChannels.filter((c) => c.name.toLowerCase().includes(filter));
		}
	...
	parseEventFromText(text) {
		...
		if (lines.length >= 2) {
			...
			const dateMatch = text.match(
				/(.+?)\s*[,·]?\s*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2},?\s+\d{4}|\d{4}-\d{2}-\d{2})/i
			);
			if (dateMatch) {
...
No text or words in the image. High quality, 16:9 aspect ratio.`;
	...
	async createScheduledEvent(guildId, eventData, options: any = {}) {
		...
		if (entityType === 3) {
			payload.entity_metadata = {
				location:
					eventData.location || eventData.entityMetadata?.location || 'To be announced',
			};
	 ...
	 */
	async createMultipleScheduledEvents(
		guildId,
		eventsText,
		location = null,
		entityType = null,
		...
		log.info(
			`[MCP] Creating ${parsedEvents.length} scheduled events in guild ${guildId}${generateImages ? ' with AI images' : ''}`
		);
	...
	async getScheduledEvents(guildId, options: any = {}) {
		...
		if (options.upcomingOnly) {
			const now = new Date();
			events = events.filter(
				(e) => e.status === 1 || e.status === 2 || new Date(e.scheduledStartTime) > now
			);
		}
	...
	previewAutomation(guildId, automationData) {
		...
		if (
			!automationData.action_config ||
			Object.keys(automationData.action_config).length === 0
		) {
	...
	async syncGuildCommandsWithDiscord(guildId, env: any = {}) {
		...
		const toDiscordFormat = (cmd) => {
			...
			if (!cmd.context_menu_user && cmd.options) {
				const options =
					typeof cmd.options === 'string' ? JSON.parse(cmd.options) : cmd.options;
		...
		if (!response.ok) {
			const errorData = await response.json().catch(() => ({}));
			throw new Error(
				`Discord command sync failed: ${response.status} - ${errorData.message || 'Unknown error'}`
			);
		}

		const registeredCommands = await response.json();
		log.info(
			`[MCP] Synced ${registeredCommands.length} commands with Discord for guild ${guildId}`
		);
		...
		try {
			switch (toolName) {
				...
				case 'get_server_stats':
					...
					try {
						...
						if (!stats) {
							return {
								success: false,
								error: 'No statistics recorded for this server yet. Stats are recorded when the dashboard is viewed.',
							};
						}
				...
				case 'cancel_local_runner_job': {
					const cancelResult = await this.cancelLocalRunnerJob(
						args.userId,
						args.jobId,
						args.reason ?? null
					);
				...
				case 'get_members_with_role':
					return {
						success: true,
						data: await this.getMembersWithRole(
							args.guildId,
							args.roleName,
							args.limit || 100,
							this.d1Database
				...
				case 'create_scheduled_event':
					return {
						success: true,
						data: await this.createScheduledEvent(
							args.guildId,
							{
								name: args.name,
				...
				case 'confirm_automation':
					return {
						success: true,
						data: await this.createAutomation(
							args.guildId,
							{
								name: args.name,
								description: args.description,
				...
				case 'confirm_command': {
					const createResult: any = await this.createCommand(
						args.guildId,
						{
							name: args.name,
							description: args.description,
...
export const MCP_TOOLS = [
	{
		name: 'list_guilds',
		description:
			'List all Discord server IDs that have data in SpaceBot. Use this first to see which servers have data.',
	},
	{
		name: 'get_event_logs',
		description:
			'Get event logs for a Discord server. Shows message creates, member joins, voice activity, role changes, etc. Supports date filtering.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			limit: 'number (optional) - Max logs to return (default: 20, max: 100)',
			category:
				'string (optional) - Filter by category: message, member, role, channel, guild, voice, reaction, etc.',
			eventType:
				'string (optional) - Filter by event type: MESSAGE_CREATE, MEMBER_JOIN, VOICE_JOIN, VOICE_LEAVE, etc.',
	...
	},
	{
		...
		description:
			"Get voice chat activity statistics for a server over a time period. Shows total voice hours, total voice joins, unique users, top voice users by time spent and by joins, and most popular voice channels. Use days=1 for today's stats.",
		parameters: {
			guildId: 'string (required) - The Discord server ID',
	...
	},
	{
		...
		description:
			'Get a comprehensive activity summary for a server. Shows message counts, member joins/leaves, voice activity, reactions, top active members, and most active channels over a time period.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
	...
	},
	{
		name: 'get_log_stats',
		description:
			'Get event log statistics for a server - total events and breakdown by category',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
		},
	},
	{
		...
		description:
			'Get automations configured for a Discord server. Can filter by active/inactive status.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			enabled:
				'boolean (optional) - Filter by status: true = active/enabled only, false = inactive/disabled only, omit for all automations',
	...
	},
	{
		...
		description:
			'Get server statistics including member count, bot count, channel count, boost level, etc. Use this to answer questions about server size or member counts.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
		},
	},
	{
		...
		description:
			"List the user's known local runner systems across all computers/servers where the runner has connected before. Returns online/offline state, token grouping, host/platform details, and pending/running job counts.",
		parameters: {
			...
			includeOffline:
				'boolean (optional) - Include offline systems as well as online ones (default: true)',
	...
	},
	{
		...
		description:
			'Get current local runner activity for the DM user: known systems, pending/running jobs, and recent runner events like connects, disconnects, job starts, and job completions/failures.',
		parameters: {
			userId: 'string (required) - Discord user ID owning the runner systems. This is injected automatically in DMs.',
	...
	},
	{
		...
		description:
			"Run a shell command on the DM user's local runner system(s). When the task targets exactly one ONLINE system, this waits for it to finish (up to waitSeconds) and returns the real output, exitCode, and status in the same call — so you can report the actual result immediately without a follow-up get_local_runner_job. When it targets multiple systems or an offline one, it only queues (the system runs it on reconnect) and returns job IDs to follow up on later. Use list_local_runners first only if you need to disambiguate which system to target.",
		parameters: {
			userId: 'string (required) - Discord user ID owning the runner systems. This is injected automatically in DMs.',
			...
			instanceNames:
				'string[] (optional) - System names/hostnames to match case-insensitively',
			targetMode: "string (optional) - 'selected' (default), 'all-online', or 'all-known'",
			priority:
				'number (optional) - Queue priority from -100 to 100 (higher runs sooner, default: 0)',
			maxAttempts: 'number (optional) - Retry budget per queued job (default: 5, max: 20)',
			timeoutSeconds:
				'number (optional) - Per-attempt timeout in seconds (default: 300, range: 30-3600)',
			capabilityRequirements:
				'object (optional) - Required runner capability flags, e.g. { screenshotAvailable: true }',
			waitForResult:
				'boolean (optional) - Force the wait behavior (true = always wait on the single target; false = never wait, just queue). Defaults to waiting only when there is exactly one online target.',
			waitSeconds:
				'number (optional) - How long to wait for the result when waiting, in seconds (default: 25, max: 90). For long commands, either raise this or set waitForResult:false and follow up with get_local_runner_job.',
		},
	},
	{
		name: 'get_local_runner_job',
		description:
			'Get full details (including command output, exit code, status, target system, timestamps) for a single local runner job owned by the DM user. Use this to follow up on a job started with start_local_runner_task, to inspect failures, or before deciding whether to cancel/retry.',
		parameters: {
	...
	},
	{
		...
		description:
			"Cancel a pending or running local runner job owned by the DM user. Use when the user asks to 'stop', 'cancel', 'kill', or 'abort' a task. Only works on jobs in 'pending' or 'running' state.",
		parameters: {
			userId: 'string (required) - Discord user ID owning the job. Injected automatically in DMs.',
	...
	},
	{
		...
		description:
			'Re-queue a failed or canceled local runner job so the owning runner system will pick it up again. The original command, working directory, target system, and limits are preserved.',
		parameters: {
			userId: 'string (required) - Discord user ID owning the job. Injected automatically in DMs.',
	...
	},
	{
		...
		description:
			'Ask a local runner workstation to detect open VS Code windows/instances and return rich discovery details including bridge endpoints and workspace folders.',
		parameters: {
			...
			waitSeconds:
				'number (optional) - How long to wait for completion when waitForResult is true (default: 20)',
			timeoutSeconds:
				'number (optional) - Runner execution timeout for this typed job (default: 90)',
		},
	},
	{
		name: 'open_vscode_workspace',
		description:
			'Open a folder/workspace path in VS Code on a target local runner system. Can open in a new VS Code window for creating a fresh instance.',
		parameters: {
			userId: 'string (required) - Discord user ID owning the runner systems. Injected automatically in DMs.',
			path: 'string (required) - Absolute workspace or folder path on the runner machine',
			newWindow:
				'boolean (optional) - Open in a new VS Code window/instance (default: false)',
			tokenId: 'number (optional) - Restrict target selection to a specific runner token',
			instanceId: 'number (optional) - Target a specific runner system ID',
			instanceName: 'string (optional) - Target by runner display name or hostname',
			bridgeUrl:
				'string (optional) - Specific VS Code bridge URL to target when multiple windows are open',
			bridgeInstanceId:
				'string (optional) - Specific VS Code bridge-reported instanceId to target',
			...
			timeoutSeconds:
				'number (optional) - Runner execution timeout in seconds (default: 120)',
		},
	},
	{
		name: 'send_vscode_copilot_message',
		description:
			'Send a prompt into Copilot Chat in VS Code from DM, optionally wait for a response, and keep prompts mirrored in normal VS Code chat UI.',
		parameters: {
			userId: 'string (required) - Discord user ID owning the runner systems. Injected automatically in DMs.',
			message: 'string (required) - Prompt text to send',
			includeResponse:
				'boolean (optional) - Return model response text in the tool result (default: true)',
			...
			requestTimeoutMs:
				'number (optional) - Timeout for the local model request in milliseconds',
			tokenId: 'number (optional) - Restrict target selection to a specific runner token',
			instanceId: 'number (optional) - Target a specific runner system ID',
			instanceName: 'string (optional) - Target by runner display name or hostname',
			...
			timeoutSeconds:
				'number (optional) - Runner execution timeout in seconds (default: 180)',
		},
	},
	{
		name: 'get_user_roles',
		description:
			"Get a user's roles in a specific Discord server. Returns the user's display name, join date, and all their roles with details like role name, color, and position. Useful for checking what roles a user has.",
		parameters: {
			guildId: 'string (required) - The Discord server ID',
	...
	},
	{
		...
		description:
			"Get all members who have a specific role in a Discord server. Use this when asked 'who has the X role?' or 'list members with Y role'. Can search by role name (case-insensitive partial match) or role ID. Returns the list of members with their display names.",
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			roleName:
				"string (required) - The role name to search for (case-insensitive, supports partial matching like 'crew' for 'Crew') or the exact role ID",
	...
	},
	{
		...
		description:
			'Get all voice and stage channels in a Discord server. Use this BEFORE creating voice or stage events to find the correct channel ID. Returns a list of voice channels and stage channels with their IDs and names.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
		},
	},
	{
		...
		description:
			'Get text channels in a Discord server. Use this to find the channel ID for a text channel by name (like #announcements, #general). Supports partial name matching.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			nameFilter:
				"string (optional) - Filter channels by name (case-insensitive partial match). For example, 'announce' will match 'announcements'.",
		},
	},
	{
		name: 'send_channel_message',
		description:
			'Send a message to a text channel in a Discord server. Use this to post messages to channels like #announcements. First use get_text_channels to find the channel ID if you only have the channel name.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			channelId: 'string (required) - The ID of the channel to send the message to',
			content:
				'string (required) - The message content to send. Can include Discord markdown and links.',
			...
			embedColor:
				'number (optional) - Embed color as integer (default: 0x5865F2 / Discord blue)',
		},
	},
	{
		...
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			nameFilter:
				"string (optional) - Filter events by name/description (case-insensitive partial match). For example, 'hockey' will match 'Men's Ice Hockey Final'.",
			upcomingOnly:
				'boolean (optional) - Set to true to only return upcoming/active events (excludes completed events)',
		},
	},
	{
		...
		parameters: {
			...
			channelId:
				'string (conditional) - Voice/Stage channel ID - REQUIRED for entityType 1 or 2. Use get_voice_and_stage_channels to find channel IDs.',
			location:
				'string (conditional) - Location or URL - REQUIRED for entityType 3 (external events)',
			generateImage:
				'boolean (optional) - Set to true to auto-generate an AI image for the event banner based on the event name',
		},
	},
	{
		name: 'create_multiple_scheduled_events',
		description:
			"Create multiple scheduled events from a text description. The text should contain event names and dates in a natural format, separated by blank lines. Supports formats like 'Event Name\\nFeb 14, 2026 · 9:10–11:30 PM'. All events will be created with the same type (external, voice, or stage). For Voice/Stage events, get the channel ID first using get_voice_and_stage_channels. Can optionally generate AI images for each event.",
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			eventsText:
				'string (required) - Text containing multiple event descriptions, separated by blank lines',
			entityType:
				'number (optional) - Event type for ALL events: 1=Stage, 2=Voice, 3=External (default: 3)',
			channelId:
				'string (conditional) - Voice/Stage channel ID - REQUIRED if entityType is 1 or 2',
			location:
				'string (conditional) - Default location for external events - REQUIRED if entityType is 3',
			generateImages:
				'boolean (optional) - Set to true to auto-generate AI images for each event banner based on their names',
		},
	},
	// Preview tools - ALWAYS use these first before creating events
	{
		...
		parameters: {
			...
			channelId:
				'string (conditional) - Voice/Stage channel ID - REQUIRED for entityType 1 or 2',
	...
	},
	{
		...
		description:
			'**ALWAYS USE THIS FIRST** before creating multiple scheduled events. Parses event text and shows the user ALL events that will be created with their dates and times. Returns a list of all parsed events for user review. The user must confirm before events are actually created.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			eventsText:
				'string (required) - Text containing multiple event descriptions, separated by blank lines',
	...
	},
	{
		...
		description:
			'Create multiple scheduled events AFTER the user has reviewed and confirmed the preview. Only use this after showing the preview and receiving user confirmation.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
	...
	// Automation creation tools - ALWAYS preview first
	{
		...
		description:
			'**ALWAYS USE THIS FIRST** before creating an automation. Shows the user what automation will be created and asks for confirmation. Returns a preview with validation errors if any. The user must confirm before the automation is actually created.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			...
			action_type:
				'string (required) - The action to perform. Valid types: SEND_MESSAGE, SEND_DM, ADD_ROLE, REMOVE_ROLE, KICK_MEMBER, BAN_MEMBER, TIMEOUT_MEMBER, LOG_TO_CHANNEL, CREATE_THREAD, ADD_REACTION, SERVER_MUTE, SERVER_UNMUTE, SERVER_DEAFEN, SERVER_UNDEAFEN, DELETE_USER_MESSAGES, DELETE_MESSAGES, CALL_WEBHOOK',
			action_config:
				"object (required) - Configuration for the action. Structure depends on action_type. For SEND_MESSAGE: {channel_id, content}. For SEND_DM: {target_user, content}. For ADD_ROLE/REMOVE_ROLE: {target_user, role_id}. For LOG_TO_CHANNEL: {channel_id, content}. For KICK_MEMBER/BAN_MEMBER: {target_user, reason}. For TIMEOUT_MEMBER: {target_user, duration_minutes}. For ADD_REACTION: {emoji}. Use 'actor' for target_user to target the user who triggered the event.",
			trigger_filters:
				"object (optional) - Conditions that must be met. Example: {channel_id: '123'} to only trigger in a specific channel, {actor_has_role: 'roleId'} to only trigger for users with a role",
	...
	},
	{
		...
		description:
			'Create an automation AFTER the user has reviewed and confirmed the preview. Only use this after showing the preview and receiving user confirmation.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			name: 'string (required) - Name for the automation',
			description: 'string (optional) - Description of what the automation does',
	...
	// Command creation tools - ALWAYS preview first
	{
		...
		description:
			'**ALWAYS USE THIS FIRST** before creating a slash command. Shows the user what command will be created and asks for confirmation. Returns a preview with validation errors if any. The user must confirm before the command is actually created and synced to Discord.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			...
			description:
				"string (required) - Short description shown in Discord's command menu (max 100 chars)",
			response_type:
				"string (optional) - How the bot responds: 'message' (plain text), 'embed' (rich embed), 'action_only' (no visible response, just runs the action). Default: 'message'",
			response_content:
				'string (optional) - The response message template. Can use template variables: {user.name}, {user.mention}, {guild.name}, {channel.name}, {option.<name>} for option values',
			response_embed:
				"object (optional) - Embed configuration for response_type='embed'. Example: {title: 'Hello!', description: 'Welcome {user.mention}', color: '#5865F2'}",
			action_type:
				"string (optional) - Action to perform when command is used. Same types as automations: SEND_MESSAGE, SEND_DM, ADD_ROLE, REMOVE_ROLE, KICK_MEMBER, BAN_MEMBER, TIMEOUT_MEMBER, LOG_TO_CHANNEL, etc. Default: 'NONE'",
			action_config:
				'object (optional) - Configuration for the action (same structure as automation action_config)',
			options:
				"array (optional) - Slash command parameters/options. Each option: {name: 'string', description: 'string', type: number, required: boolean, choices?: [{name, value}]}. Types: 3=Text, 4=Integer, 5=Boolean, 6=User, 7=Channel, 8=Role, 10=Number",
			ephemeral:
				'boolean (optional) - If true, response is only visible to the user who ran the command (default: false)',
			default_member_permissions:
				"string (optional) - Discord permission bitfield to restrict who can use the command. '8' = Admin only, '32' = Manage Server, null = everyone",
	...
	},
	{
		...
		description:
			'Create a slash command AFTER the user has reviewed and confirmed the preview. Only use this after showing the preview and receiving user confirmation. The command will be auto-synced to Discord so it appears in the / menu.',
		parameters: {
			guildId: 'string (required) - The Discord server ID',
			name: 'string (required) - Command name (lowercase, alphanumeric/hyphens)',
			description: 'string (required) - Command description',
			response_type:
				"string (optional) - 'message', 'embed', or 'action_only'. Default: 'message'",
...
export function formatToolsForPrompt(includeCatalogue = true) {
	...
	prompt +=
		'**BEFORE you can tell the user ANY statistics, counts, automations, commands, logs, or settings, you MUST call a tool first.**\n\n';
