...
interface ListUserWorkflowsOptions {
	limit?: number;
	enabledOnly?: boolean;
}

interface DispatchWorkflowPayload {
	job_type?: string;
	runner_strategy?: string;
	capability_requirements_json?: any;
	model_preferences_json?: any;
	label?: string;
	payload_json?: any;
	command?: string;
	working_dir?: string;
	...
	target_runner_token_id?: number;
	target_runner_instance_id?: number;
}
...
function normalizeModelPreferences(value) {
	...
	const modeRaw =
		typeof value.mode === 'string' ? value.mode.trim().toLowerCase() : 'first_success';
...
async function resolvePinnedTarget(db, userId, workflow, payload) {
	...
	const tokenId =
		Number.isInteger(overrideTokenId) && overrideTokenId > 0
			? overrideTokenId
	...
	if (!validation.success || !validation.tokenId) {
		return {
			success: false,
			error: 'Pinned strategy requires a valid target runner token or instance',
		};
	}
...
async function resolveOnlineTargets(db, userId, workflow, payload, strategy) {
	...
	const overrideTokenId = Number(payload?.target_runner_token_id);
	const overrideInstanceId = Number(payload?.target_runner_instance_id);
	const pinnedTokenId = workflow.target_runner_token_id
		? Number(workflow.target_runner_token_id)
		: null;
	const pinnedInstanceId = workflow.target_runner_instance_id
		? Number(workflow.target_runner_instance_id)
		: null;

	const candidates = onlineInstances.filter((instance) => {
		if (
			Number.isInteger(overrideInstanceId) &&
			overrideInstanceId > 0 &&
			instance.id !== overrideInstanceId
		) {
			return false;
		}
		if (
			Number.isInteger(overrideTokenId) &&
			overrideTokenId > 0 &&
			instance.tokenId !== overrideTokenId
		) {
			return false;
		}
		if (
...
async function validateTargets(db, userId, tokenId, instanceId) {
	...
	let resolvedTokenId =
		Number.isInteger(parsedTokenId) && parsedTokenId > 0 ? parsedTokenId : null;
	const resolvedInstanceId =
		Number.isInteger(parsedInstanceId) && parsedInstanceId > 0 ? parsedInstanceId : null;
	...
	if (resolvedInstanceId) {
		const instance = await db
			.prepare(
				'SELECT id, runner_token_id FROM local_runner_instances WHERE id = ? AND user_id = ?'
			)
...
export async function listUserWorkflows(db, userId, options: ListUserWorkflowsOptions = {}) {
	if (!db || !userId) return [];
...
export async function createUserWorkflow(db, userId, input) {
	...
	const allowedRunnerTokens = normalizeJsonIdList(
		input?.allowed_runner_token_ids_json,
		'allowed_runner_token_ids_json'
	);
	if (allowedRunnerTokens.error) return { success: false, error: allowedRunnerTokens.error };
	const allowedRunnerInstances = normalizeJsonIdList(
		input?.allowed_runner_instance_ids_json,
		'allowed_runner_instance_ids_json'
	);
	if (allowedRunnerInstances.error)
		return { success: false, error: allowedRunnerInstances.error };
...
export async function updateUserWorkflow(db, userId, workflowId, patch) {
	...
	const nextJobType =
		patch?.job_type === undefined ? current.job_type : normalizeJobType(patch.job_type);
	...
	const runnerStrategy =
		patch?.runner_strategy === undefined
			? current.runner_strategy || DEFAULT_RUNNER_STRATEGY
			: normalizeRunnerStrategy(patch?.runner_strategy);
	if (!runnerStrategy) return { success: false, error: 'Unsupported runner_strategy' };

	const allowedRunnerTokens = normalizeJsonIdList(
		patch?.allowed_runner_token_ids_json,
		'allowed_runner_token_ids_json'
	);
	if (allowedRunnerTokens.error) return { success: false, error: allowedRunnerTokens.error };
	const allowedRunnerInstances = normalizeJsonIdList(
		patch?.allowed_runner_instance_ids_json,
		'allowed_runner_instance_ids_json'
	);
	if (allowedRunnerInstances.error)
		return { success: false, error: allowedRunnerInstances.error };
	...
	const targetValidation = await validateTargets(
		db,
		userId,
		patch?.target_runner_token_id === undefined
			? current.target_runner_token_id
			: patch.target_runner_token_id,
		patch?.target_runner_instance_id === undefined
			? current.target_runner_instance_id
			: patch.target_runner_instance_id
	);
	if (!targetValidation.success) return targetValidation;
	...
	const capabilityRequirementsJson = capability.present
		? capability.value
		: current.capability_requirements_json
			? JSON.stringify(current.capability_requirements_json)
			: null;
	const allowedRunnerTokensJson = allowedRunnerTokens.present
		? allowedRunnerTokens.value
		: Array.isArray(current.allowed_runner_token_ids_json)
			? JSON.stringify(current.allowed_runner_token_ids_json)
			: null;
	const allowedRunnerInstancesJson = allowedRunnerInstances.present
		? allowedRunnerInstances.value
		: Array.isArray(current.allowed_runner_instance_ids_json)
			? JSON.stringify(current.allowed_runner_instance_ids_json)
			: null;
	const modelPreferencesJson = modelPreferences.present
		? modelPreferences.value
		: current.model_preferences_json
			? JSON.stringify(current.model_preferences_json)
			: null;
...
export async function dispatchWorkflowJob(
	...
	const effectiveJobType = payload?.job_type
		? normalizeJobType(payload.job_type)
		: workflow.job_type;
	...
	const capabilityRequirements =
		payload?.capability_requirements_json !== undefined
	...
	for (const target of targets.targets) {
		for (const variant of variants) {
			const payloadJson = mergePayloadWithProviderChain(
				payload?.payload_json,
				variant.providerChain
			);
