 ...
 *  so they share one allowance rather than each having their own: 1,500 rows x
 *  ~5 writes (4 indexes + the row) = ~7.5k each. Combined across all four
...
function toSqlDateTime(value: string | Date = new Date()) {
...
async function updateCheckpoint(db, guildId, checkpointType, lastEventId) {
  ...
  `
		...
		.bind(guildId, checkpointType, lastEventId, lastEventId)
		.run();
}
...
async function closeVoiceSessions(db, sessions, closedAt, leaveEventId = null) {
    ...
    `
			...
			.bind(closedAtSql, leaveEventId, closedAtSql, session.id)
			.run();
	}
...
export async function reconcileVoiceSessions(
	...
	for (const [userId, sessions] of openByUser.entries()) {
		...
		const matchingSession = sessions.find(
			(session) => session.channel_id === activeState.channel_id
		);
		if (matchingSession) {
...
export async function processVoiceSessions(db, guildId) {
  ...
  `
		...
		.bind(guildId, checkpoint.lastEventId)
		.all();
	...
	for (const event of events.results) {
		...
		if (event.event_type === 'VOICE_JOIN') {
			...
			const matchingSession = openSessions.find(
				(session) => session.channel_id === event.channel_id
			);
      ...
      `
			...
			sessionsClosed += await closeVoiceSessions(
				db,
				openSessions,
				event.created_at,
				event.id
...
export async function buildHourlyStats(db, guildId, options: Record<string, any> = {}) {
    ...
    `
      ...
      `
			...
			const bootstrapStart = new Date(
				Date.now() - HOURLY_BOOTSTRAP_MAX_DAYS * 24 * 60 * 60 * 1000
			)
				.toISOString()
				.slice(0, 19)
				.replace('T', ' ');
			startTime =
				earliest?.earliest && Date.parse(earliest.earliest) > Date.parse(bootstrapStart)
					? earliest.earliest
					: bootstrapStart;
		}
    ...
    `
      ...
      `
				...
				.bind(guildId, period.period_start, period.period_end)
				.first();
      ...
      `
				...
				.bind(guildId, period.period_start, period.period_start)
				.first();

			const baselineCount = baseline?.count || 0;

			const peakConcurrent = await db
				.prepare(
					`
      ...
      `
				...
				.bind(guildId, period.period_start, period.period_end, baselineCount, baselineCount)
				.first();
...
export async function buildDailyStats(db, guildId, options: Record<string, any> = {}) {
    ...
    `
		...
		const effectiveDailyStart = !repair
			...
			: !lastAggregated?.last_period ||
				  Date.parse(lastAggregated.last_period) > Date.parse(dailyRepairStart)
				? dailyRepairStart
				: lastAggregated.last_period;

		// Get days that have hourly data and aren't today
		const daysToProcess = await db
			.prepare(
				`
      SELECT 
    ...
    `
			...
			.bind(guildId, effectiveDailyStart)
			.all();
      ...
      `
				...
				.bind(guildId, day.day_start, day.day_end)
				.first();
 ...
 */
export async function getAggregatedStats(db, guildId, options: Record<string, any> = {}) {
	if (!db) return [];
    ...
    `
		...
		return {
			...
			uniqueUsers:
				...
				aggregated?.unique_users ||
				partial?.voice_unique_users ||
				0,
...
export async function cleanupOldData(db, guildId = null) {
	...
	// Batched and budgeted: these tables had never been pruned, so the first run
	// after scheduling faces months of backlog. A single unbounded DELETE there
	// can spend the whole daily D1 write allowance (one row-write per row plus
	// one per index — 5 and 5 respectively here) and take live logging with it.
	// The backlog drains over successive nights instead.
	const sessions = await deleteInBatches(db, {
		table: 'voice_sessions',
		where: guildId
			? `guild_id = ? AND left_at IS NOT NULL AND left_at < datetime('now', '-90 days')`
			: `left_at IS NOT NULL AND left_at < datetime('now', '-90 days')`,
		binds: guildId ? [guildId] : [],
		budget: VOICE_SESSION_DELETE_BUDGET,
		label: 'voice_sessions (>90d)',
	});

	// Hourly aggregates older than 30 days; the daily rollups they fed are kept
	// forever and are what the charts actually read.
	const hourly = await deleteInBatches(db, {
		table: 'aggregated_stats',
		where: guildId
			? `guild_id = ? AND period_type = 'hourly' AND period_start < datetime('now', '-30 days')`
			: `period_type = 'hourly' AND period_start < datetime('now', '-30 days')`,
		binds: guildId ? [guildId] : [],
		budget: HOURLY_AGGREGATE_DELETE_BUDGET,
		label: 'aggregated_stats hourly (>30d)',
	});

	return {
		sessionsDeleted: sessions.deleted,
		aggregatesDeleted: hourly.deleted,
		budgetExhausted: sessions.budgetExhausted || hourly.budgetExhausted,
	};
}
...
export async function closeStaleVoiceSessions(db, guildId = null) {
    ...
    `
		...
		if (closed > 0) {
			log.warn(
				`[Stats] Closed ${closed} stale orphaned voice sessions${guildId ? ` for ${guildId}` : ' globally'}`
			);
		}
...
export async function getMemberGrowthChart(db, guildId, period = '30d', timezone = null) {
    ...
    `
		...
		const rawByDate = new Map<any, any>(rawData.map((row) => [row.date, row]));
...
export async function getVoiceActivityChart(db, guildId, period = '30d', timezone = null) {
    ...
    `
		...
		if (existingToday) {
			...
			existingToday.uniqueUsers = Math.max(
				existingToday.uniqueUsers,
				partial.voice_unique_users
			);
			existingToday.peakConcurrent = Math.max(
				existingToday.peakConcurrent || 0,
				partial.voice_peak_concurrent || 0
			);
		} else {
			rawData.push({
				date: today,
		...
		return fillDateGaps(
			rawData,
			days,
			{ totalMinutes: 0, totalHours: 0, uniqueUsers: 0, peakConcurrent: 0 },
			timezone
		);
	} catch (error) {
...
export async function getMessageActivityChart(db, guildId, period = '30d', timezone = null) {
    ...
    `
		...
		return fillDateGaps(rawData, days, { messageCount: 0, messageUniqueUsers: 0 }, timezone);
	} catch (error) {
