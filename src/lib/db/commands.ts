 ...
 * @property {boolean} ephemeral - Response only visible to user (static default)
 * @property {string|null} [ephemeral_option] - Option ref that drives ephemerality per-invocation (e.g. "private", "!public"); null = use the static boolean
...
export const COMMAND_TEMPLATE_VARIABLES = {
	...
	'target_message.author_name':
		"Right-clicked message author's username (message context menu only)",
...
export async function createCommand(db, command) {
	...
	try {
      ...
      INSERT INTO commands (
        ...
        options, ephemeral, ephemeral_option, defer,
...
export async function updateCommand(db, id, updates: Record<string, any> = {}) {
    ...
    `
		...
		if (updates.ephemeral_option !== undefined) {
			fields.push('ephemeral_option = ?');
			values.push(updates.ephemeral_option || null);
		}
		...
		if (updates.response_embed !== undefined) {
			...
			values.push(updates.response_embed ? JSON.stringify(updates.response_embed) : null);
		}
...
export async function getCommandLogs(
	db,
	guildId,
	options: { commandId?: number; limit?: number } = {}
) {
	if (!db) return [];
...
function parseCommand(row) {
	const parsed = {
		...
		ephemeral_option: row.ephemeral_option ?? null,
		defer: !!row.defer,
...
function applyBuiltInOverride(command, override) {
	...
	return {
		...
		default_member_permissions:
			override.default_member_permissions === null ||
			override.default_member_permissions === undefined
 ...
 */
export async function setBuiltInCommandOverride(
	db,
	guildId,
	commandId,
	updates: { enabled?: boolean | null; default_member_permissions?: string | null } = {}
) {
	if (!db) {
		return { success: false, error: 'Database not available' };
	}
    ...
    `
		...
		const nextDefaultMemberPermissions =
			updates.default_member_permissions === undefined
    ...
    `
			...
			.bind(guildId, commandId, nextEnabled, nextDefaultMemberPermissions)
			.run();
...
export async function getBuiltInCommandsForGuild(db, guildId) {
	...
	return builtInCommands.map((command) =>
		applyBuiltInOverride(command, overrideMap.get(command.id))
	);
}
...
export async function deleteBuiltInCommand(db, id) {
	...
	try {
		...
		await db
			.prepare('DELETE FROM commands WHERE id = ? AND guild_id = ?')
			.bind(id, BUILT_IN_GUILD_ID)
			.run();
	...
	try {
		for (const cmd of BUILT_IN_COMMAND_DEFAULTS) {
			const existing = await db
				...
				.bind(BUILT_IN_GUILD_ID, cmd.name)
				.first();
 ...
 *
 * @param {Object} command - command record (reads `ephemeral`, `ephemeral_option`)
 * @param {Object} interaction - raw Discord interaction (reads `data.options`)
 * @returns {boolean}
 */
