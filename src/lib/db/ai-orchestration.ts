...
export async function getAIJobById(db, jobId) {
	...
	const row = await db.prepare(`SELECT * FROM ai_jobs WHERE id = ?`).bind(jobId).first();
...
export async function listAIJobsForUser(
	...
	const status =
		typeof options.status === 'string' && options.status.trim() ? options.status.trim() : null;
...
export async function listAIJobsForGuild(
	db,
	guildId,
		...
		const update = await db
         ...
         WHERE id = ?
           AND status = 'running'`
			...
			.bind(
				retryAtExpr,
				String(errorText || 'Unknown error').slice(0, 2000),
				stringifyNullableJson(decision),
				jobId
			)
			.run();
...
/**
 * Watchdog requeue policy (2026-07-24, the Queues-limit incident).
 *
 * The 5-minutely sweep used to re-enqueue every due pending job with no
 * backoff and no cap — a stuck job cost ~860 queue ops/day against the free
 * tier's 10,000/day. The policy now: exponential backoff between requeues
 * (5 → 10 → 20 → … minutes, capped at a day) and a terminal cap — a job the
 * watchdog has requeued MAX_WATCHDOG_REQUEUES times without progress is
 * failed_terminal instead of looped forever. The requeue counter is the
 * existing `job.requeued` audit trail, so no schema change.
 */
export const MAX_WATCHDOG_REQUEUES = 8;

export function watchdogBackoffMinutes(requeueCount) {
	const n = Number.isFinite(requeueCount) && requeueCount > 0 ? requeueCount : 0;
	return Math.min(5 * 2 ** n, 1440);
}

/** How many times the watchdog has already requeued this job. */
export async function countAIJobRequeues(db, jobId) {
	if (!db || !jobId) return 0;
	try {
		const row = await db
			.prepare(
				`SELECT COUNT(*) AS n FROM ai_job_events
         WHERE job_id = ? AND event_type = 'job.requeued'`
			)
			.bind(jobId)
			.first();
		return Number(row?.n ?? 0);
	} catch (err) {
		log.warn('[AI Autopilot] Failed to count requeues:', err?.message || err);
		return 0;
	}
}

/**
 * Push a pending job's next_retry_at into the future so the sweep stops
 * re-enqueueing it every cycle while it waits to execute.
 */
export async function deferAIJobRetry(db, jobId, minutes) {
	if (!db || !jobId) return { success: false };
	const safeMinutes = clampInt(minutes, 5, 1, 1440);
	try {
		await db
			.prepare(
				`UPDATE ai_jobs
         SET next_retry_at = datetime('now', '+' || ? || ' minutes'),
             updated_at = datetime('now')
         WHERE id = ? AND status = 'pending'`
			)
			.bind(safeMinutes, jobId)
			.run();
		return { success: true };
	} catch (err) {
		log.warn('[AI Autopilot] Failed to defer job retry:', err?.message || err);
