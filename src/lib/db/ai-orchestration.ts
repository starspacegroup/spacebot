...
export async function listAIJobsForUser(db, userId, options: { limit?: number; offset?: number; status?: string } = {}) {
  if (!db || !userId) return [];
...
export async function listAIJobsForGuild(db, guildId, options: { limit?: number; status?: string } = {}) {
