...
function parseRecordedAt(recordedAt) {
	...
	const normalized = recordedAt.includes('T') ? recordedAt : recordedAt.replace(' ', 'T');
...
export async function updateMemberCount(db, guildId, eventType, isBot = false) {
	...
	try {
		...
		const newHumanCount =
			newBotCount !== null ? Math.max(0, newCount - newBotCount) : latest.human_count;
    ...
    `
		...
		log.debug(
			`[Stats] Updated member count for ${guildId}: ${latest.member_count} -> ${newCount} (${eventType}${isBot ? ', bot' : ''})`
		);
 ...
 */
export async function syncServerStatsIfStale(
	db,
	guildId,
	botToken,
	...
	const { existingStats = null, maxAgeMinutes = DEFAULT_STATS_REFRESH_MINUTES } = options;

	const latest = existingStats ?? (await getLatestServerStats(db, guildId));
	const shouldRefresh =
		!latest || latest.member_count === 0 || isServerStatsStale(latest, maxAgeMinutes);
	...
	try {
		...
		if (!discordStats || discordStats.member_count <= 0) {
			...
			return {
				latest,
				refreshed: false,
				error: 'Discord API returned no valid member count',
			};
		}
		...
		if (!saveResult.success) {
			return {
				latest,
				refreshed: false,
				error: saveResult.error || 'Failed to record server stats',
			};
		}
	...
	try {
      ...
      `
		...
		return {
			...
			day:
				dayAgo?.member_count !== null && dayAgo?.member_count !== undefined
					? currentCount - dayAgo.member_count
					: 0,
			week:
				weekAgo?.member_count !== null && weekAgo?.member_count !== undefined
					? currentCount - weekAgo.member_count
					: 0,
			month:
				monthAgo?.member_count !== null && monthAgo?.member_count !== undefined
			...
			dayHuman:
				currentHuman !== null &&
				dayAgo?.human_count !== null &&
				dayAgo?.human_count !== undefined
					? currentHuman - dayAgo.human_count
					: null,
			weekHuman:
				currentHuman !== null &&
				weekAgo?.human_count !== null &&
				weekAgo?.human_count !== undefined
					? currentHuman - weekAgo.human_count
					: null,
			monthHuman:
				currentHuman !== null &&
				monthAgo?.human_count !== null &&
				monthAgo?.human_count !== undefined
...
export async function pruneOldStats(db, guildId = null) {
	...
	// Batched and budgeted rather than one unbounded DELETE: this had never run,
	// so the first pass after scheduling faces months of rows, and server_stats
	// carries 5 indexes (~6 row-writes per deleted row) against D1's ~100k/day
	// write cap. The backlog drains over successive nights.
	const pruned = await deleteInBatches(db, {
		table: 'server_stats',
		where: guildId
			? `guild_id = ? AND recorded_at < datetime('now', '-90 days')`
			: `recorded_at < datetime('now', '-90 days')`,
		binds: guildId ? [guildId] : [],
		budget: SERVER_STATS_DELETE_BUDGET,
		label: 'server_stats (>90d)',
	});
 ...
 */
export async function fetchGuildStatsFromDiscord(
	botToken,
	guildId,
	options: Record<string, any> = {}
) {
	...
	try {
		...
		log.debug(
			`[Stats] Fetched guild ${guildId}: ${memberCount} members, ${botCount ?? '?'} bots, ${humanCount ?? '?'} humans`
		);
