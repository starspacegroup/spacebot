...
function normalizeTriggerEvents(values) {
	...
	const normalized = values.map((value) => normalizeTriggerEvent(value)).filter(Boolean);
...
function triggerMatchesEvent(trigger, normalizedEventType) {
	...
	if (normalizedTrigger.endsWith(':*') || normalizedTrigger.endsWith('_*')) {
		const prefix = normalizedTrigger.slice(0, -2);
		return normalizedEventType === prefix || normalizedEventType.startsWith(`${prefix}_`);
	}
...
export const FILTER_TYPES = {
	channel_id: {
		...
		applicableEvents: ['MESSAGE_', 'VOICE_', 'THREAD_', 'REACTION_', 'CHANNEL_PINS_UPDATE'],
	},
	not_channel_id: {
		type: 'channel',
...
export async function createAutomation(db, automation) {
	...
	try {
		...
		const cleanActionConfig: Record<string, any> = {};
		if (automation.action_config) {
			// Deep clone to strip any reactive wrappers
			if (automation.action_config.actions) {
				cleanActionConfig.actions = automation.action_config.actions.map((action) => ({
					type: action.type,
    ...
    `
			...
			.bind(
				...
				automation.trigger_filters ? JSON.stringify(automation.trigger_filters) : null,
...
export async function updateAutomation(db, id, updates, guildId = null) {
	...
	try {
		...
		if (updates.trigger_filters !== undefined) {
			...
			values.push(updates.trigger_filters ? JSON.stringify(updates.trigger_filters) : null);
		}
	...
	try {
      ...
      `
		...
		const parsed = {
			...
			trigger_filters: result.trigger_filters ? JSON.parse(result.trigger_filters) : null,
...
export async function getAutomations(
	db,
	guildId,
	options: { limit?: number; offset?: number; eventType?: string; enabled?: boolean } = {}
) {
	if (!db) {
	...
	log.info(
		`[DB] getAutomations called: guildId=${guildId}, eventType=${eventType}, enabled=${enabled}, limit=${limit}, offset=${offset}`
	);
    ...
    `
		...
		log.info(
			`[DB] getAutomations query results: count=${countResult?.total}, rows=${results.results?.length}`
		);
		...
		return {
			automations: (results.results || []).map((a) => {
				...
				parsed.trigger_events = parseStoredTriggerEvents(a.trigger_events, a.trigger_event);
				parsed.trigger_event = parsed.trigger_events[0] || null;
				return parsed;
			}),
...
export async function getTriggeredAutomations(db, guildId, eventType) {
    ...
    `
		...
		const matched = allEnabled.filter((a) => {
			const triggerEvents = parseStoredTriggerEvents(a.trigger_events, a.trigger_event);
...
export async function getAutomationLogs(
	db,
	guildId,
	options: { limit?: number; offset?: number; automationId?: number; success?: boolean } = {}
) {
		...
		const enrichedLogs = parsedLogs.map((entry, index) => {
			...
			return {
				...
				trigger_data: {
					...
					actor_discriminator:
						actor.actor_discriminator || entry.trigger_data.actor_discriminator || '0',
				},
			};
		});
...
export async function toggleAutomation(db, id, guildId, enabled) {
	...
	try {
      ...
      `
				...
				.bind(enabled ? 1 : 0, parseInt(id), guildId)
				.run();
		} else {
			await db
				.prepare(
      ...
      `
				...
				.bind(enabled ? 1 : 0, String(id), guildId)
				.run();
		}
