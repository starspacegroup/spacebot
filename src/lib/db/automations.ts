...
export async function createAutomation(db, automation) {
  ...
  try {
    ...
    let cleanActionConfig: Record<string, any> = {};
 ...
 */
export async function getAutomations(db, guildId, options: { limit?: number; offset?: number; eventType?: string; enabled?: boolean } = {}) {
  if (!db) {
 ...
 */
export async function getAutomationLogs(db, guildId, options: { limit?: number; offset?: number; automationId?: number; success?: boolean } = {}) {
