...
<div class="flow-builder" class:compact ontouchmove={handleTouchMove} ontouchend={handleTouchEnd}>
	{#if actions.length === 0}
		<div class="flow-empty">
			...
			<button type="button" class="flow-add-first" onclick={addAction}> + Add Action </button>
		</div>
	{:else}
		<div class="flow-list">
			{#each actions as action, index}
				...
				<div
					...
					class:dragging={dragIndex === index ||
						(touchDrag?.index === index && touchDrag?.moved)}
				...
				>
					...
					<!-- svelte-ignore a11y_no_static_element_interactions -->
					<div
						class="flow-action-header"
						onclick={() => action.type && toggleCollapse(index)}
					>
						<!-- svelte-ignore a11y_click_events_have_key_events -->
						<!-- svelte-ignore a11y_no_static_element_interactions -->
						<span
						...
						{:else}
							<span class="flow-action-name flow-action-placeholder"
								>Select action...</span
							>
						{/if}
												...
												<div class="flow-field">
													...
													{:else if config.type === 'color_rules'}
														<div class="flow-color-rules">
															...
															<button
																type="button"
																class="flow-btn-add-rule"
																onclick={() =>
																	addColorRule(action, configKey)}
																>+ Add color rule</button
															>
														</div>
													...
													{:else if config.type === 'delay'}
														<div class="flow-delay-picker">
															<div class="delay-presets">
																{#each [{ value: '5m', label: '5 min' }, { value: '15m', label: '15 min' }, { value: '30m', label: '30 min' }, { value: '1h', label: '1 hour' }, { value: '2h', label: '2 hours' }, { value: '6h', label: '6 hours' }, { value: '12h', label: '12 hours' }, { value: '1d', label: '1 day' }, { value: '3d', label: '3 days' }, { value: '7d', label: '7 days' }] as preset}
																	<button
																		type="button"
																		class="delay-preset-btn"
																		class:active={action.config[
																			configKey
																		] === preset.value}
																		onclick={() => {
																			action.config[
															...
															{#if action.config[configKey]}
																<span class="delay-preview"
																	>⏱ Message will be sent {formatDelay(
																		action.config[configKey]
																	)} after trigger</span
																>
															{/if}
