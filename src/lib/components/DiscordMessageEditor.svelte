<script>
	...
	const maskedLinkFormat = { icon: '🔗+', label: 'Masked Link', prefix: '[text](', suffix: ')', shortcut: '' };
	...
	function applyFormat(prefix, suffix, lineStart = false) {
		...
		if (lineStart) {
			...
			if (linePrefix.trim().length > 0 && start === end) {
				...
				newText = value.substring(0, lineStartPos) + prefix + value.substring(lineStartPos, start) + selectedText + suffix + value.substring(end);
...
<div class="discord-editor">
	...
	<div class="editor-toolbar">
		...
		<div class="dropdown-container">
			<button type="button" class="toolbar-btn" title="Headings">
				H
			</button>
		...
		<div class="dropdown-container">
			<button type="button" class="toolbar-btn" title="Blocks & Lists">
				≡
			</button>
			<div class="dropdown-menu compact-dropdown">
		...
		<div class="insert-buttons">
			...
			{#if roles && roles.length > 0}
				<div class="dropdown-container">
					<button type="button" class="toolbar-btn insert-btn" title="Insert role mention">
						@ Role
					</button>
			...
			<div class="dropdown-container">
				<button type="button" class="toolbar-btn insert-btn" title="Insert template variable">
					{'{}'} Variable
				</button>
				...
				{#each activeItems as item, i}
					...
					>
						{#if mentionType === 'channel'}
							...
							<span>{item.name}</span>
						{:else}
							{@const roleColor = item.color ? `#${item.color.toString(16).padStart(6, '0')}` : '#99aab5'}
							<span class="role-dot" style="background-color: {roleColor}"></span>
							<span>{item.name}</span>
						{/if}
					</button>
	...
	<div class="editor-help">
		<span>Type <kbd>#</kbd> for channels, <kbd>@&</kbd> for roles, <kbd>{'{'}</kbd> for variables</span>
		<span class="help-divider">|</span>
		<span><kbd>**bold**</kbd> <kbd>*italic*</kbd> <kbd># heading</kbd> <kbd>- list</kbd> <kbd>[text](url)</kbd></span>
	</div>
</div>
