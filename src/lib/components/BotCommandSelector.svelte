<script lang="ts">
	...
	const availableCommands: any[] = $derived(
		selectedBot ? Object.values(selectedBot.commands) : []
	);

	// Get selected command info
	const selectedCommand = $derived(
		selectedBot && commandValue ? selectedBot.commands[commandValue] : null
	);
	...
	function handleCommandKeydown(event) {
		...
		switch (event.key) {
			case 'ArrowDown':
				event.preventDefault();
				commandHighlightedIndex = Math.min(
	...
	function handleClickOutside(event) {
		if (
			botDropdownRef &&
			!botDropdownRef.contains(event.target) &&
			botInputRef &&
			!botInputRef.contains(event.target)
		) {
			isBotDropdownOpen = false;
		}
		if (
			commandDropdownRef &&
			!commandDropdownRef.contains(event.target) &&
			commandInputRef &&
			!commandInputRef.contains(event.target)
		) {
			isCommandDropdownOpen = false;
		}
	}
		...
		<div class="selector-container">
			{#if selectedBot}
				...
				<div class="selected-item">
					<span class="bot-icon" style="color: {selectedBot.color}"
						>{selectedBot.icon}</span
					>
				...
				<div class="dropdown" bind:this={botDropdownRef}>
						...
						{#each filteredBots as bot, index}
							...
							>
								...
								<span class="command-count"
									>{Object.keys(bot.commands).length} commands</span
								>
							</button>
						{/each}
					{/if}
					...
					<div class="dropdown-footer">
						<span class="footer-hint"
							>💡 Don't see your bot? You can still filter by bot ID directly.</span
						>
					</div>
				</div>
			{/if}
						...
						{:else}
							...
							>
								...
								<div class="bot-info">
									...
									<span class="bot-desc"
										>Match any /{selectedBot.name.toLowerCase()} command</span
									>
								</div>
							</button>
