<script lang="ts">
	...
	function addButton(rowIndex) {
		...
		rows = rows.map((row, i) => 
			i === rowIndex ? [...row, createDefaultButton()] : row
		);
	}
		...
		<div class="button-row-editor">
			<div class="row-header">
				...
				<div class="row-controls">
					...
					{#if row.length < 5}
						<button type="button" class="btn-add-inline" onclick={() => addButton(rowIndex)} title="Add button">
							+
						</button>
					{/if}
					{#if rows.length > 1}
						<button type="button" class="btn-remove-inline" onclick={() => removeRow(rowIndex)} title="Remove row">
							×
						</button>
					{/if}
				</div>
			...
			<div class="button-list">
				{#each row as btn, btnIndex}
					...
					<div 
						...
						class:dragging={dragState?.rowIndex === rowIndex && dragState?.btnIndex === btnIndex}
						draggable="true"
					...
					>
						...
						<!-- svelte-ignore a11y_no_static_element_interactions -->
						<div class="button-summary" onclick={() => toggleExpand(rowIndex, btnIndex)}>
							...
							<button type="button" class="btn-expand" title={isExpanded(rowIndex, btnIndex) ? 'Collapse' : 'Expand'}>
								{isExpanded(rowIndex, btnIndex) ? '▾' : '▸'}
							</button>
							<button type="button" class="btn-remove-btn" onclick={(e) => { e.stopPropagation(); removeButton(rowIndex, btnIndex); }} title="Remove button">
								×
							</button>
						</div>
							...
							<div class="button-config">
								<div class="config-grid">
										...
										<div class="style-picker">
											{#each BUTTON_STYLES as style}
												<button 
													...
													onclick={() => updateButton(rowIndex, btnIndex, 'style', style.value)}
													title={style.label}
												>
													{style.label}
												</button>
									...
									<div class="config-field">
										...
										<input 
											...
											maxlength="32"
										/>
										<span class="field-hint">Unique identifier for this button</span>
									</div>
									{#if btn.style === 5}
										<!-- svelte-ignore a11y_label_has_associated_control -->
								...
								{#if btn.style !== 5}
									<div class="button-actions-section">
										<h4>⚡ Click Actions</h4>
										<p class="section-hint">Actions to execute when this button is clicked</p>
										<ActionFlowBuilder
...
<style>
	...
	.btn-add-inline, .btn-remove-inline {
		width: 1.5rem;
		height: 1.5rem;
		border-radius: 50%;
		border: none;
