...
import { getMessagePurgeSettings, DEFAULT_MAX_BATCHES } from '../server/message-purge-settings.js';
import {
	deleteUserMessages,
 ...
 * run — the superadmin "max scan batches" lookback setting (× 100 messages per
 * batch), the same knob that bounds the durable purge workflow. One setting
 * governs every message-lookback delete, not just the /spam command. Falls back
 * to the default when no DB is available (e.g. tests).
 */
async function getMessageScanCeiling(db: any): Promise<number> {
	const { maxBatches } = await getMessagePurgeSettings(db);
	return (maxBatches || DEFAULT_MAX_BATCHES) * 100;
}
...
export function resolveNumberValue(configValue, event, defaultValue = null) {
	...
	if (typeof configValue === 'object') {
		...
		if (configValue.source === 'option' && configValue.optionName) {
			return resolveNumberValue(`option:${configValue.optionName}`, event, defaultValue);
		}
	}
 ...
 */
async function sendInteractionFollowup(applicationId, interactionToken, payload, _botToken) {
...
function resolveEmbedColor(actionConfig, context) {
	const defaultColor =
		actionConfig.embed_color && /^#[0-9a-fA-F]{6}$/.test(actionConfig.embed_color)
...
export function matchesFilters(
	...
	for (const [filterType, filterValue] of Object.entries(filters) as [string, any][]) {
		switch (filterType) {
			case 'channel_id':
			...
			case 'actor_has_role':
				...
				if (!isAllSentinel(filterValue)) {
					...
					if (!requiredRoles.some((role) => context.actorRoles?.includes(role)))
						return false;
				}
				break;
			...
			case 'target_has_role':
				...
				if (!isAllSentinel(filterValue)) {
					...
					if (!requiredRoles.some((role) => context.targetRoles?.includes(role)))
						return false;
				}
				break;

			case 'content_contains':
				if (!event.details?.content?.toLowerCase().includes(filterValue.toLowerCase()))
					return false;
				break;
			...
			case 'min_account_age_days':
				if (context.accountAgeDays !== undefined && context.accountAgeDays < filterValue)
					return false;
				break;
			...
			case 'command_result':
				...
				if (filterValue && filterValue !== 'any') {
					...
					log.debug(
						`[Filter] command_result: expected=${filterValue}, detected=${detectedResult}, embedTexts=${JSON.stringify(
							event.details?.embedTexts
						)}`
					);
			...
			case 'github_action':
				...
				if (
					...
					if (actual !== expected && actualSuffix !== expected && status !== expected)
						return false;
				}
				break;
 ...
 * @returns {Object}
 */
export function buildContext(event, guildInfo: Record<string, any> = {}) {
	...
	const github = {
		...
		branch:
			githubDetails.branch || githubDetails.head_branch || githubDetails.base_branch || '',
		title:
			githubDetails.title ||
	...
	try {
		switch (action_type) {
			case 'DELETE_USER_MESSAGES': {
				...
				const { totalDeleted, channelResults } = await deleteUserMessages(discord, {
					...
					maxAgeDays,
					skipPinned,
					maxScan: await getMessageScanCeiling(db),
				});

				return {
					success: true,
					result: { totalDeleted, channelResults },
				};
			}
			...
			case 'DELETE_MESSAGES': {
				...
				const { totalDeleted, channelResults } = await deleteMessagesByUser(discord, {
					guildId: event.guild_id,
					channelIds,
					userId,
					limit,
			...
			case 'DELETE_MENTIONED_MESSAGES': {
				...
				const { totalDeleted, channelResults } = await deleteMentionedMessages(discord, {
					guildId: event.guild_id,
					channelIds,
					userId,
					limit,
			...
			case 'SEND_MESSAGE': {
				...
				if (action_config.ephemeral && event.application_id && event.interaction_token) {
					...
					const payload: Record<string, any> = { flags: 64 };
			...
			case 'SEND_MESSAGE_WITH_BUTTONS': {
				...
				if (action_config.ephemeral && event.application_id && event.interaction_token) {
					...
					const payload: Record<string, any> = { flags: 64, components };
					if (action_config.embed) {
						const embedColor = resolveEmbedColor(action_config, context);
						const thumbnailUrl = action_config.embed_thumbnail_url
							? processTemplate(action_config.embed_thumbnail_url, context)
				...
				const messagePayload: Record<string, any> = { components };
			...
			case 'SEND_STATS_WIDGET_IMAGE': {
				...
				const signingSecret =
					context.widget_signing_secret ||
					(typeof process !== 'undefined' ? process.env?.DISCORD_PUBLIC_KEY : undefined);
			...
			case 'ADD_ROLE': {
				...
				const guild = await discord.guilds.fetch(event.guild_id).catch(() => null);
				if (!guild) {
			...
			case 'KICK_MEMBER': {
				const userId = resolveTargetUser(action_config, event);
				const reason = processTemplate(action_config.reason || 'Automated action', context);
			...
			case 'ADD_REACTION': {
				...
				if (!messageId) {
					return {
						success: false,
						error: 'No message ID available - this action requires a message event',
					};
				}
 ...
 */
export async function processAutomations(event, db, discord, guildInfo = {}, filterContext = {}) {
	...
	try {
		...
		const automations = await getTriggeredAutomations(db, event.guild_id, event.event_type);
		...
		for (const automation of automations) {
			...
			if (!matchesFilters(event, automation.trigger_filters, filterContext)) {
				log.info(
					`[Automation] "${automation.name}" — filters not matched, skipping (filters: ${JSON.stringify(
			...
			for (let i = 0; i < actionsToExecute.length; i++) {
				...
				const result = await executeAction(actionAutomation, event, context, discord, db);
				actionResults.push({
					actionIndex: i,
...
export function buildButtonComponents(buttonRows, guildId, sourceId) {
	...
	for (const row of rows.slice(0, 5)) {
		...
		for (const btn of row.slice(0, 5)) {
			...
			const style = typeof btn.style === 'number' ? btn.style : BUTTON_STYLES[btn.style] || 2;
...
export function createAutomationEngine(db) {
	return async function handleEvent(event, discord, guildInfo = {}, filterContext = {}) {
