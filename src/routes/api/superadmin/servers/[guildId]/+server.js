...
import { getServerPlan, upsertServerPlan, deleteServerPlan, PLAN_TIERS } from "$lib/db/server-plans.js";
...
export async function PATCH({ cookies, platform, params, request }) {
  ...
  if (planBefore.price_cents !== plan.price_cents) changes.push(`price: $${((planBefore.price_cents || 0) / 100).toFixed(2)} → $${((plan.price_cents || 0) / 100).toFixed(2)}`);
  if (planBefore.max_commands !== plan.max_commands) changes.push(`commands limit: ${planBefore.max_commands ?? '∞'} → ${plan.max_commands ?? '∞'}`);
  if (planBefore.max_automations !== plan.max_automations) changes.push(`automations limit: ${planBefore.max_automations ?? '∞'} → ${plan.max_automations ?? '∞'}`);
  if (planBefore.expires_at !== plan.expires_at) changes.push(`expiry: ${planBefore.expires_at || 'none'} → ${plan.expires_at || 'none'}`);

  const description = planBefore.plan !== plan.plan
    ? `Plan changed from ${planBefore.plan} to ${plan.plan} by admin`
    : `Plan settings updated by admin`;

  await addBillingEvent(db, {
    guild_id: params.guildId,
    event_type: planBefore.plan !== plan.plan
      ? (PLAN_TIERS[plan.plan]?.price_cents > (PLAN_TIERS[planBefore.plan]?.price_cents || 0)
