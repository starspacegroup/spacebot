...
export async function PATCH({ cookies, platform, params, request }) {
	...
	const { change_note, ...patch } = body || {};
	const result = await updateSuperadminWorkflowTemplate(db, templateId, userId, patch, {
		changeNote: change_note ? String(change_note) : undefined,
	});
	if (!result.success) {
		const status =
			result.error === 'Workflow not found'
				? 404
				: result.error === 'A workflow with this slug already exists'
					? 409
					: 400;
