...
export async function GET({ request, url, cookies, platform }) {
	...
	if (!CLIENT_ID) {
		console.error('[OAuth] DISCORD_CLIENT_ID not configured. Platform env keys:', 
			(platform as any)?.env ? Object.keys((platform as any).env) : 'platform.env is undefined');
		return json({ 
			error: "DISCORD_CLIENT_ID not configured",
			debug: {
				hasPlatform: !!platform,
				hasEnv: !!(platform as any)?.env,
				envKeys: (platform as any)?.env ? Object.keys((platform as any).env).filter(k => !k.includes('SECRET') && !k.includes('TOKEN')) : []
			}
		}, { status: 500 });
	}
