	...
	return {
		...
		recurrenceRule: event.recurrence_rule
			? {
					...
					byWeekday:
						event.recurrence_rule.by_weekday?.map((d) => RECURRENCE_WEEKDAY[d]) || [],
...
export async function GET({ params, cookies, platform, url }) {
	...
	try {
		...
		events.sort(
			(a, b) =>
				(new Date(a.scheduledStartTime) as any) - (new Date(b.scheduledStartTime) as any)
		);
...
export async function POST({ params, cookies, platform, request }) {
	...
	try {
		...
		if (body.entityType === 3) {
			eventPayload.entity_metadata = {
				location: body.entityMetadata?.location || 'To be announced',
			};
	...
	try {
		...
		if (action) {
			...
			if (body.scheduledStartTime !== undefined)
				updatePayload.scheduled_start_time = body.scheduledStartTime;
			if (body.scheduledEndTime !== undefined)
				updatePayload.scheduled_end_time = body.scheduledEndTime;
			if (body.channelId !== undefined) updatePayload.channel_id = body.channelId;
			if (body.entityMetadata !== undefined)
				updatePayload.entity_metadata = body.entityMetadata;
