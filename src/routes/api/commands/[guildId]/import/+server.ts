...
export async function POST({ params, request, cookies, platform }) {
	...
	try {
		...
		if (!body.format || body.format !== 'spacebot-commands') {
			return json(
				{ error: 'Invalid file format. Expected a SpaceBot commands export file.' },
				{ status: 400 }
			);
		}
		...
		for (const command of body.commands) {
			...
			if (
				...
				!ACTION_TYPES[command.action_type]
			) {
				results.push({
					name: command.name,
					success: false,
					error: `Invalid action type: ${command.action_type}`,
				});
				errorCount++;
				continue;
			}
			...
			try {
				...
				const result = await createCommand(db, {
					...
					ephemeral_option: command.ephemeral_option || null,
		...
		return json(
			{ error: 'Failed to import commands. Make sure the file is valid JSON.' },
			{ status: 500 }
		);
	}
}
