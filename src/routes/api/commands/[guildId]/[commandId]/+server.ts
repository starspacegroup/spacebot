	...
	try {
		...
		if (existing.is_built_in) {
			...
			if (!checkIsSuperAdmin(userId, platform)) {
				return json(
					{ error: 'Only superadmins can edit built-in commands' },
					{ status: 403 }
				);
			}
		}
		...
		if (body.action === 'toggle') {
			...
			if (body.enabled) {
				try {
					const activeCommands = await getGuildCommands(db, guildId, {
						enabledOnly: true,
					});
		...
		if (body.action_type && body.action_type !== 'NONE' && !ACTION_TYPES[body.action_type]) {
			return json(
				{ error: `Invalid action_type: ${body.action_type}` },
				{
					status: 400,
				}
			);
