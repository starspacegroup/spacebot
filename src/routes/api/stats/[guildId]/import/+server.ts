...
export async function POST({ params, request, cookies, platform }) {
	...
	try {
		...
		if (body.format !== 'spacebot-stats') {
			return json(
				{ error: 'Invalid file format. Expected a SpaceBot stats export file.' },
				{ status: 400 }
			);
		}
		...
		return json(
			{ error: 'Failed to import stats data: ' + (error.message || String(error)) },
			{ status: 500 }
		);
	}
}
