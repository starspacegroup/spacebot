...
export async function GET({ params, cookies, platform, url }) {
	...
	try {
      ...
      `
		...
		if (format === 'csv') {
			return new Response(statsExportToCsv(exportData), {
				headers: {
					'Content-Type': 'text/csv; charset=utf-8',
					'Content-Disposition': `attachment; filename="spacebot-stats-${guildId}.csv"`,
				},
			});
		}
		...
		return json(exportData, {
			headers: {
				'Content-Disposition': `attachment; filename="spacebot-stats-${guildId}.json"`,
			},
		});
	} catch (error) {
