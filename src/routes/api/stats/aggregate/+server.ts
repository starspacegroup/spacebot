...
import { getGuildsWithLogs } from '$lib/server/cron-jobs.js';
...
function verifyAuth(request, platform) {
	const cronSecret = (platform as any)?.env?.CRON_SECRET || process.env.CRON_SECRET;
	...
	const internalKey = (platform as any)?.env?.INTERNAL_API_KEY || process.env.INTERNAL_API_KEY;
...
export async function POST({ request, platform }) {
	...
	try {
		...
		for (const guildId of guilds) {
			try {
				// Incremental only — the 7-day repair sweep runs in the daily job.
				const { hourly, daily } = await runStatsAggregation(db, guildId, { repair: false });
				...
				if (hourly.success && daily.success) {
					...
					if (hourly.error)
						results.errors.push({ guildId, type: 'hourly', error: hourly.error });
					if (daily.error)
						results.errors.push({ guildId, type: 'daily', error: daily.error });
				}
			} catch (error) {
				results.guildsFailed++;
		...
		log.info(
			`[StatsAggregation] Completed: ${results.guildsProcessed} guilds, ${results.totalHourlyPeriods} hourly periods, ${results.totalDailyPeriods} daily periods in ${duration}ms`
		);
...
export async function GET({ platform }) {
	...
	return json({
		...
		method: 'POST',
		description:
			'Runs hourly stats aggregation for all guilds with recent activity. Smart enough to skip already-processed data.',
