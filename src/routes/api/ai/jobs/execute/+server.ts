...
export async function POST({ request, platform }) {
  ...
  const env = {
    ...
    // Local Ollama gate (dev-only).
    AI_PROVIDER: getEnv(platform, "AI_PROVIDER"),
    OLLAMA_MODEL: getEnv(platform, "OLLAMA_MODEL"),
    OLLAMA_BASE_URL: getEnv(platform, "OLLAMA_BASE_URL"),
    DB: db,
  };
  ...
  try {
    ...
    const decisionRequestsRunner = decision?.providerSwitchHint?.to === "runner";
    const shouldTryRunnerFallback = Boolean(decision.retry) && (
      decisionRequestsRunner ||
