...
export async function GET({ request, platform }) {
  ...
  const executionContext = (platform as any)?.context;
  ...
  server.addEventListener("error", () => {
    closed = true;
    if (state.instanceId) {
      executionContext?.waitUntil(disconnectRunnerInstance(db, state.instanceId));
    }
  });
  ...
  executionContext?.waitUntil(jobPushLoop());
