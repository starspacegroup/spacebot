...
import {
	recordGatewayBenchmark,
	getBenchmarkStats,
	getBenchmarkChartData,
	getRecentBenchmarks,
} from '$lib/db/gateway-benchmarks.js';
...
export async function POST({ request, platform }) {
	...
	try {
		...
		const intervalStr = await getGlobalSetting(
			db,
			'benchmark_interval_seconds',
			String(DEFAULT_BENCHMARK_INTERVAL)
		);
...
export async function PATCH({ request, cookies, platform }) {
	...
	try {
		...
		const validIntervals = [
			30, 60, 120, 300, 600, 1800, 3600, 7200, 14400, 28800, 43200, 86400,
		];
