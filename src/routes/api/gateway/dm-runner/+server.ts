...
interface DmJobContext {
	managedGuilds?: unknown;
	selectedGuild?: { id?: unknown; [key: string]: unknown } | null;
	selectedGuildId?: unknown;
}
...
function inferDmJob(
	...
	const selectedGuild =
		context.selectedGuild && typeof context.selectedGuild === 'object'
			? context.selectedGuild
			: null;
	const selectedGuildId =
		typeof context.selectedGuildId === 'string' && context.selectedGuildId.trim()
			? context.selectedGuildId.trim()
			: typeof selectedGuild?.id === 'string'
				? selectedGuild.id.trim()
				: null;
...
export async function POST({ request, platform }) {
	...
	const db = (platform as { env?: Record<string, any> } | undefined)?.env?.DB;
	if (!db) {
	...
	const selectedGuild =
		body?.selectedGuild && typeof body.selectedGuild === 'object' ? body.selectedGuild : null;
	const selectedGuildId =
		typeof body?.selectedGuild?.id === 'string'
			? body.selectedGuild.id.trim()
			: typeof body?.selectedGuildId === 'string'
				? body.selectedGuildId.trim()
				: '';
	...
	// DM chat takeover is opt-in at the account level. Only screenshots (which
	// must produce a binary artifact on the machine) and the explicit
	// "prefer local runner for DMs" preference route a text DM to the runner.
	//
	// Merely naming a runner ("how many monitors on Dirac?", "run tests on Dirac")
	// no longer dispatches: the cloud assistant is grounded on the same runner
	// inventory + machine context and can execute commands synchronously via
	// start_local_runner_task, so it answers and acts better than routing the
	// whole turn to the runner's (often much weaker) local model.
	...
	if (!result.success) {
		return json(
			{ dispatched: false, reason: 'dispatch_failed', error: result.error },
			{ status: 500 }
		);
	}
