<script>
	...
	const isAdminGrantedPro = $derived(
		plan.plan === 'pro' && !plan.stripe_subscription_id && !isProActive && !isCanceling && !isPastDue
	);
...
<div class="account-page">
	...
	<header class="page-header">
		...
		<div class="header-content">
			...
			<p class="header-desc">Manage your plan, usage, and subscription for {data.guild?.name || 'this server'}</p>
		</div>
	</header>
	...
	<section class="overview-section">
		<div class="overview-grid">
			<div class="overview-card server-card">
				<div class="server-identity">
					{#if data.guild?.icon}
						<img 
							class="server-icon" 
							src="https://cdn.discordapp.com/icons/{data.serverId}/{data.guild.icon}.webp?size=64" 
							alt="{data.guild.name}" 
						/>
					{:else}
				...
				<div class="overview-card billing-summary-card">
					...
					<div class="billing-details">
						...
						{#if data.nextBillingDate}
							<div class="detail-row">
								<span class="detail-label">{isCanceling ? 'Access until' : 'Next billing'}</span>
					...
					<div class="billing-actions">
						{#if isProActive}
							<button class="btn btn-secondary btn-sm" onclick={() => billingAction('portal')} disabled={loading}>
								{loading ? '...' : 'Manage Payment'}
							</button>
							<button class="btn btn-danger-outline btn-sm" onclick={() => billingAction('cancel')} disabled={loading}>
								{loading ? '...' : 'Cancel'}
							</button>
						{:else if isCanceling}
							<button class="btn btn-primary btn-sm" onclick={() => billingAction('reactivate')} disabled={loading}>
								{loading ? '...' : 'Reactivate'}
							</button>
							<button class="btn btn-secondary btn-sm" onclick={() => billingAction('portal')} disabled={loading}>
						...
						{:else if isPastDue}
							<button class="btn btn-primary btn-sm" onclick={() => billingAction('portal')} disabled={loading}>
								{loading ? '...' : 'Fix Payment'}
							</button>
						{/if}
					</div>
			...
			{:else if isAdminGrantedPro}
				<div class="overview-card billing-summary-card">
					<h3>Plan Info</h3>
					<p class="admin-granted-note">Pro features were granted by an administrator. Subscribe below to manage your own billing and ensure uninterrupted access.</p>
				</div>
			{/if}
		</div>
	</section>
	...
	<section class="usage-section">
			...
			<div class="usage-card">
				...
				<div class="usage-bar-container">
					{#if plan.max_api_keys}
						<div class="usage-bar">
							<div 
								class="usage-bar-fill {usageStatus(usage.apiKeys, plan.max_api_keys)}" 
		...
		<div class="retention-info">
			<div class="retention-item">
				...
				<span class="retention-value">{plan.log_retention_days ? `${plan.log_retention_days} days` : 'Unlimited'}</span>
			</div>
			<div class="retention-item">
					...
					<div class="plan-card-footer">
						{#if data.stripeConfigured}
							{#if isAdminGrantedPro}
								<p class="plan-note" style="margin-bottom: 0.5rem;">Your Pro access was granted manually. Subscribe to manage your own billing.</p>
							{/if}
							<button 
								...
								disabled={loading}
							>
								{loading ? 'Redirecting to Stripe...' : isAdminGrantedPro ? 'Subscribe to Pro' : 'Upgrade to Pro'}
							</button>
						{:else}
...
<style>
	...
	.usage-bar-fill.unlimited {
		background: linear-gradient(90deg, var(--color-primary), var(--color-accent-hover, var(--color-primary)));
		opacity: 0.3;
	}
