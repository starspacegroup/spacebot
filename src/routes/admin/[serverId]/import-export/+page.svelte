<script lang="ts">
	...
	async function exportAll() {
		...
		try {
			...
			const parts = [];
			if (counts.automations)
				parts.push(
					`${counts.automations} automation${counts.automations !== 1 ? 's' : ''}`
				);
			if (counts.commands)
				parts.push(`${counts.commands} command${counts.commands !== 1 ? 's' : ''}`);
			const statsTotal =
				(counts.server_stats || 0) +
				(counts.aggregated_stats || 0) +
	...
	async function importFile(file) {
		...
		try {
			...
			if (parsed.format === 'spacebot-automations') {
				...
				importError =
					"Invalid file format. This doesn't appear to be a SpaceBot export file.";
				importing = false;
				return;
			}
...
<div class="import-export-page">
	...
	<header class="page-header">
		<div class="header-content">
			...
			<p class="header-subtitle">
				Share automations and commands between servers, and export stats data for backup
			</p>
		</div>
	</header>
	...
	<section class="section-card">
		<div class="section-header">
			...
			<p class="section-desc">
				Download automations, commands, or stats as a JSON file to share or back up
			</p>
		</div>
			...
			<div class="export-all-bar">
				<div class="export-all-info">
					...
					<div>
						...
						<span class="export-all-desc"
							>Export everything in one file — automations, commands, settings & stats</span
						>
					</div>
				</div>
				<button class="btn btn-primary" disabled={exporting} onclick={exportAll}>
					Export All Data
				</button>
			</div>
		{/if}
			...
			{:else}
				<div class="item-select-bar">
					<label class="select-all-label">
						<input
							type="checkbox"
							checked={selectedAutomations.size === automationCount}
							onchange={toggleAllAutomations}
						/>
						<span
							>{selectedAutomations.size === automationCount
								? 'Deselect All'
								: 'Select All'}</span
						>
					</label>
				...
				<div class="export-items">
					{#each data.automations as automation}
						...
						<label class="export-item" class:selected={selectedAutomations.has(id)}>
							<input
								type="checkbox"
								checked={selectedAutomations.has(id)}
								onchange={() => toggleAutomation(id)}
							/>
			...
			{:else}
				<div class="item-select-bar">
					<label class="select-all-label">
						<input
							type="checkbox"
							checked={selectedCommands.size === commandCount}
							onchange={toggleAllCommands}
						/>
						<span
							>{selectedCommands.size === commandCount
								? 'Deselect All'
								: 'Select All'}</span
						>
					</label>
				...
				<div class="export-items">
					{#each data.commands as command}
						...
						<label class="export-item" class:selected={selectedCommands.has(id)}>
							<input
								type="checkbox"
								checked={selectedCommands.has(id)}
								onchange={() => toggleCommand(id)}
							/>
	...
	<section class="section-card">
		<div class="section-header">
			...
			<p class="section-desc">
				Upload a SpaceBot export file to import automations, commands, or backup settings
			</p>
		</div>
		...
		{#if !importResult}
			...
			<div class="import-notes">
				...
				<ul>
					<li>
						Accepts full backups, or individual automation and command export files —
						the format is auto-detected
					</li>
					<li>
						Server-specific references (channels, roles, users) are <strong
							>automatically cleared</strong
						> during import for automations/commands
					</li>
					<li>
						Automations and commands are imported as <strong>disabled</strong> — enable them
						after reviewing
					</li>
					<li>
						Commands with duplicate names will fail — rename existing commands first
					</li>
					<li>Backup files with stats data will skip the stats section here</li>
					{#if data?.isSuperAdmin}
						<li>
							Use <a href="/admin/superadmin/stats-import">Superadmin Stats Import</a> to
							restore stats exports
						</li>
					{/if}
				</ul>
			</div>
		{:else}
			...
			<div class="import-results">
				<div
					class="result-summary {importResult.failed > 0 || !importResult.success
						? 'partial'
						: 'success'}"
				>
					<span class="result-icon"
						>{importResult.failed > 0 || !importResult.success ? '⚠️' : '✅'}</span
					>
				...
				{#if importResult.type === 'backup' && importResult.results}
					...
					{#if r.automations?.results?.length > 0}
						<div class="backup-result-section">
							<h4>
								⚡ Automations ({r.automations.imported} imported{r.automations
									.failed > 0
									? `, ${r.automations.failed} failed`
									: ''})
							</h4>
								...
								{#each r.automations.results as result}
									<div class="result-item {result.success ? 'success' : 'error'}">
										...
										{#if result.success && result.needs_configuration}
											<span class="result-needs-config"
												>⚠ needs: {result.needs_configuration}</span
											>
										{/if}
									</div>
								{/each}
					...
					{#if r.commands?.results?.length > 0}
						<div class="backup-result-section">
							<h4>
								💬 Commands ({r.commands.imported} imported{r.commands.failed > 0
									? `, ${r.commands.failed} failed`
									: ''})
							</h4>
							<div class="result-details">
					...
					{#if r.skipped?.statsRecords > 0}
						<div class="backup-result-section">
							<h4>📊 Stats Data</h4>
							<p class="backup-result-note">
								Skipped {r.skipped.statsRecords} stats record{r.skipped
									.statsRecords !== 1
									? 's'
									: ''}. Restore stats from the Superadmin panel.
							</p>
						</div>
					{/if}
