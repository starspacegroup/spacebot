<script lang="ts">
	...
	function formatTriggerSummary(triggerData) {
		...
		const items = [];
		if (triggerData.actor_name)
			items.push({ label: 'Actor', value: triggerData.actor_name, id: triggerData.actor_id });
		if (triggerData.channel_name)
			items.push({
				label: 'Channel',
				value: `#${triggerData.channel_name}`,
				id: triggerData.channel_id,
			});
		if (triggerData.target_name)
			items.push({
				label: 'Target',
				value: triggerData.target_name,
				id: triggerData.target_id,
			});
	...
	function formatActionConfig(config) {
		...
		for (const [key, value] of Object.entries(config)) {
			...
			displayPairs.push({
				label,
				value: displayValue.length > 80 ? displayValue.slice(0, 77) + '...' : displayValue,
			});
		}
						...
						<div class="log-item {log.success ? 'log-success' : 'log-error'}">
							...
							>
								...
								<div class="log-info">
									<span class="log-name"
										>{log.automation_name ||
											`Automation #${log.automation_id}`}</span
									>
								...
								<div class="log-details">
									<div class="log-meta-row">
										{#if log.execution_time_ms}
											<span class="log-meta-item"
												>⏱️ {log.execution_time_ms}ms</span
											>
										{/if}
										{#if log.created_at}
											<span class="log-meta-item"
												...
												<div class="trigger-summary-items">
													{#each summary as item}
														<span class="trigger-summary-item">
															<span class="trigger-summary-key"
																>{item.label}:</span
															>
									...
									{#if log.action_result && Array.isArray(log.action_result)}
										<div class="log-actions-section">
											<span class="log-detail-label"
												>Actions ({log.action_result.length})</span
											>
											<div class="log-actions-list">
												{#each log.action_result as action, i}
													<div
														class="log-action-item {action.success
															? 'action-success'
															: 'action-error'}"
													>
														<div class="log-action-header">
															<span class="action-status-dot"
																>{action.success ? '✓' : '✕'}</span
															>
														...
														{#if action.result}
															<div class="action-result-data">
																<pre
																	class="action-result-json">{JSON.stringify(
																		action.result,
																		null,
																		2
																	)}</pre>
									...
									{#if log.trigger_data}
										<details class="log-raw-details">
											<summary class="log-raw-summary"
												>Raw Trigger Data</summary
											>
											<pre class="log-detail-json">{JSON.stringify(
													formatLogData(log.trigger_data),
													null,
													2
												)}</pre>
										</details>
					...
					<div class="automation-card {automation.enabled ? '' : 'disabled'}">
						<div class="automation-header">
							<div class="automation-triggers">
								{#each visibleTriggers as trigger}
									...
									<div
										...
										style="--category-color: {triggerCategoryInfo.color}"
									>
										<span class="trigger-icon"
											>{trigger.icon || triggerCategoryInfo.icon}</span
										>
							...
							>
								<input
									type="hidden"
									name="id"
									value={automation.public_id || automation.id}
								/>
						...
						<div class="automation-footer">
							...
							<div class="automation-actions">
								<a
									href="/admin/{selectedGuildId}/automations/{automation.public_id ||
										automation.id}"
			...
			{#if data.pagination.totalPages > 1}
				<div class="pagination">
					{#if data.pagination.hasPrev}
						<a href="?page={data.pagination.page - 1}" class="btn btn-secondary"
							>← Previous</a
						>
					{/if}
...
<style>
	...
	.action-result-json {
		background: color-mix(
			in srgb,
			var(--bg-primary, #202225) 80%,
			var(--color-success, #43b581) 8%
		);
		border-radius: 4px;
