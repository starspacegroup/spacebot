...
export async function load({ cookies, platform, parent, params }) {
  ...
  const hasAccessToServer =
    isSuperAdmin || adminGuilds.some((g) => g.id === serverId);
  ...
  const availableIntegrations = integrations.filter(
    (i) => i.connected_at || i.guild_enabled,
  );
...
export const actions = {
  ...
  enableIntegration: async ({ request, cookies, platform, params }) => {
    ...
    try {
      await syncGuildCommands(db, serverId, platform?.env);
    } catch (err) {
  ...
  disableIntegration: async ({ request, cookies, platform, params }) => {
    ...
    log.info(
      `[Integrations] Disabling integration ${integrationId} for server ${serverId}`,
    );
  ...
  updateConfig: async ({ request, cookies, platform, params }) => {
    ...
    const result = await updateGuildIntegrationConfig(
      db,
      serverId,
      integrationId,
      config,
