<script lang="ts">
	...
	function toggleWeekday(dayIndex) {
		if (createRecurrenceWeekdays.includes(dayIndex)) {
			...
			createRecurrenceWeekdays = [...createRecurrenceWeekdays, dayIndex].sort(
				(a, b) => a - b
			);
		}
	}
	...
	function initializeEditForm(event, isFullSeries) {
		...
		editStartTime = event.scheduledStartTime
			? toLocalDateTimeString(new Date(event.scheduledStartTime))
			: '';
		editEndTime = event.scheduledEndTime
			? toLocalDateTimeString(new Date(event.scheduledEndTime))
			: '';
		...
		if (event.recurrenceRule) {
			...
			editRecurrenceWeekdays =
				event.recurrenceRule.byWeekday?.map((name) => weekdayNameToIndex[name]) || [];
						...
						<div class="event-details">
							<div class="detail-row">
								...
								<div class="detail-content">
									...
									<span class="detail-value"
										>{formatDateTime(event.scheduledStartTime)}</span
									>
									<span class="detail-relative"
										>{formatRelativeTime(event.scheduledStartTime)}</span
									>
								</div>
							</div>
							...
							{#if event.scheduledEndTime}
								<div class="detail-row">
									...
									<div class="detail-content">
										...
										<span class="detail-value"
											>{formatDateTime(event.scheduledEndTime)}</span
										>
									</div>
								</div>
							{/if}
							...
							{:else if event.channelId}
								<div class="detail-row">
									<span class="detail-icon"
										>{event.entityType === 1 ? '🎭' : '🔊'}</span
									>
									<div class="detail-content">
										<span class="detail-label">Channel</span>
										<span class="detail-value"
											>{getChannelName(event.channelId)}</span
										>
									</div>
								</div>
							{/if}
							...
							<div class="detail-row">
								...
								<div class="detail-content">
									...
									<span class="detail-value"
										>{event.userCount} member{event.userCount !== 1
											? 's'
											: ''}</span
									>
								</div>
			...
			>
				...
				>
					<div class="modal-body">
								...
								{:else}
									<label class="image-upload-label" for="create-image">
										...
										<span class="upload-hint"
											>PNG, JPEG, GIF, or WebP (max 10MB)</span
										>
									</label>
								{/if}
								<input
						...
						{#if createEntityType === 3}
							<div class="form-group">
								...
								<span class="form-hint"
									>This can be a URL, address, or any location description</span
								>
							</div>
						{:else}
							<div class="form-group">
								...
								<select
									id="create-channel"
									name="channelId"
									bind:value={createChannelId}
									required
								>
									<option value="">Select a channel</option>
									{#each data.voiceChannels.filter( (c) => (createEntityType === 1 ? c.type === 'stage' : c.type === 'voice') ) as channel}
						...
						<div class="form-group">
							<label for="create-end"
								>End Time {createEntityType === 3 ? '*' : ''}</label
							>
							<input
								type="datetime-local"
								id="create-end"
						...
						{#if createIsRecurring}
							...
							<div class="recurrence-options">
									...
									<div class="form-group">
										...
										<div class="interval-input">
											...
											<span class="interval-label">
												{createRecurrenceFrequency === 3
													? 'day(s)'
													: createRecurrenceFrequency === 2
														? 'week(s)'
														: createRecurrenceFrequency === 1
															? 'month(s)'
															: 'year(s)'}
											</span>
										</div>
								...
								{#if createRecurrenceFrequency === 2}
									<div class="form-group">
										...
										>
											{#each [0, 1, 2, 3, 4, 5, 6] as dayIndex}
												<button
													type="button"
													class="weekday-btn"
													class:selected={createRecurrenceWeekdays.includes(
														dayIndex
													)}
													onclick={() => toggleWeekday(dayIndex)}
													aria-pressed={createRecurrenceWeekdays.includes(
														dayIndex
													)}
												>
										...
										<input
											...
											value={createRecurrenceWeekdays.join(',')}
										/>
									</div>
								{/if}
								...
								<div class="form-group">
									...
									<span class="form-hint"
										>Leave blank for indefinite recurrence</span
									>
								</div>
							</div>
						{/if}
					...
					<div class="choice-options">
						<button class="choice-card" onclick={editNextOccurrence}>
							...
							<div class="choice-content">
								...
								<div class="choice-subtitle">
									Only change the time for the upcoming event
								</div>
							</div>
						</button>
						...
						<button class="choice-card choice-card-primary" onclick={editEntireSeries}>
							...
							<div class="choice-content">
								...
								<div class="choice-subtitle">
									Change name, description, channel, image, and recurrence
									settings
								</div>
							</div>
						</button>
					...
					<div class="modal-header-content">
						...
						{#if editingEvent.recurrenceRule}
							<span class="modal-badge"
								>{editFullSeries ? '🔄 Series' : '📅 Single'}</span
							>
						{/if}
					</div>
									...
									{:else}
										<label class="image-upload-label" for="edit-image">
											...
											<span class="upload-text"
												>Click to upload cover image</span
											>
											<span class="upload-hint"
												>PNG, JPEG, GIF, or WebP (max 10MB)</span
											>
							...
							<div class="form-section">
								...
								{#if editEntityType === 3}
									<div class="form-group">
										...
										/>
										<span class="form-hint"
											>A link, address, or any location description</span
										>
									</div>
								{:else}
									<div class="form-group">
										...
										>
											{#each data.voiceChannels.filter( (c) => (editEntityType === 1 ? c.type === 'stage' : c.type === 'voice') ) as channel}
												<option value={channel.id}>
													{channel.type === 'stage' ? '🎭' : '🔊'}
													{channel.name}
												</option>
										...
										<span class="form-hint"
											>Select a different channel to move the event</span
										>
									</div>
								{/if}
							</div>
							...
							<div class="occurrence-info">
								<div class="occurrence-header">
									...
									<div class="occurrence-details">
										...
										<span class="occurrence-subtitle"
											>Editing only the next scheduled occurrence</span
										>
									</div>
								</div>
							</div>
										...
										<div class="form-row">
											...
											<div class="form-group">
												...
												<div class="interval-input">
													...
													/>
													<span class="interval-label">
														{editRecurrenceFrequency === 3
															? 'day(s)'
															: editRecurrenceFrequency === 2
																? 'week(s)'
																: editRecurrenceFrequency === 1
																	? 'month(s)'
																	: 'year(s)'}
													</span>
												</div>
										...
										{#if editRecurrenceFrequency === 2}
											<div class="form-group">
												...
												>
													{#each [0, 1, 2, 3, 4, 5, 6] as dayIndex}
														<button
															type="button"
															class="weekday-btn"
															class:selected={editRecurrenceWeekdays.includes(
																dayIndex
															)}
															onclick={() =>
																toggleEditWeekday(dayIndex)}
															aria-pressed={editRecurrenceWeekdays.includes(
																dayIndex
															)}
														>
															{getWeekdayShort(dayIndex)}
												...
												<input
													type="hidden"
													name="recurrenceWeekdays"
													value={editRecurrenceWeekdays.join(',')}
												/>
											</div>
										{/if}
