		...
		events = rawEvents.map((event) => ({
			...
			recurrenceRule: event.recurrence_rule
				? {
						...
						frequencyName:
							RECURRENCE_FREQUENCY[event.recurrence_rule.frequency] || 'Unknown',
						interval: event.recurrence_rule.interval,
						byWeekday:
							event.recurrence_rule.by_weekday?.map((d) => WEEKDAY_NAMES[d]) || [],
...
export const actions = {
	create: async ({ request, platform, params }) => {
		...
		const recurrenceFrequency = parseInt(
			(formData.get('recurrenceFrequency') as string) || '2'
		);
		...
		const recurrenceCount = formData.get('recurrenceCount')
			? parseInt(formData.get('recurrenceCount') as string)
			: null;
		...
		if (scheduledEndTime)
			eventPayload.scheduled_end_time = new Date(scheduledEndTime as string).toISOString();
		...
		if (recurrenceEnabled) {
			...
			if (recurrenceFrequency === 2 && recurrenceWeekdaysStr) {
				const weekdays = (recurrenceWeekdaysStr as string)
					.split(',')
					.filter(Boolean)
					.map((d) => parseInt(d));
	...
	update: async ({ request, platform, params }) => {
		...
		if (scheduledStartTime)
			updatePayload.scheduled_start_time = new Date(
				scheduledStartTime as string
			).toISOString();
		if (scheduledEndTime)
			updatePayload.scheduled_end_time = new Date(scheduledEndTime as string).toISOString();
		...
		if (recurrenceEnabled) {
			const recurrenceRule: RecurrenceRulePayload = {
				start: scheduledStartTime
					? new Date(scheduledStartTime as string).toISOString()
