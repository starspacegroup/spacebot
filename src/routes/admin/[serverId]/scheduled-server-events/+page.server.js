...
export const actions = {
  create: async ({ request, cookies, platform, params }) => {
    ...
    const entityType = parseInt(formData.get("entityType") || "3");
    ...
    const recurrenceFrequency = parseInt(formData.get("recurrenceFrequency") || "2");
    const recurrenceInterval = parseInt(formData.get("recurrenceInterval") || "1");
    const recurrenceWeekdaysStr = formData.get("recurrenceWeekdays") || "";
    const recurrenceCount = formData.get("recurrenceCount") ? parseInt(formData.get("recurrenceCount")) : null;
    ...
    if (scheduledEndTime) eventPayload.scheduled_end_time = new Date(scheduledEndTime).toISOString();
    ...
    if (recurrenceEnabled) {
      const recurrenceRule = {
        start: new Date(scheduledStartTime).toISOString(),
      ...
      if (recurrenceFrequency === 2 && recurrenceWeekdaysStr) {
        const weekdays = recurrenceWeekdaysStr.split(",").filter(Boolean).map(d => parseInt(d));
  ...
  update: async ({ request, cookies, platform, params }) => {
    ...
    if (scheduledStartTime) updatePayload.scheduled_start_time = new Date(scheduledStartTime).toISOString();
    if (scheduledEndTime) updatePayload.scheduled_end_time = new Date(scheduledEndTime).toISOString();
    ...
    if (recurrenceEnabled) {
      const recurrenceRule = {
        start: scheduledStartTime ? new Date(scheduledStartTime).toISOString() : new Date().toISOString(),
        frequency: recurrenceFrequency,
        interval: recurrenceInterval,
      };
