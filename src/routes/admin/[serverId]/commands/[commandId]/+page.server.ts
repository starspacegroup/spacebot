...
export const actions = {
	update: async ({ request, platform, params, cookies }) => {
		...
		if (isBuiltIn) {
			const defaultMemberPermissions =
				(formData.get('default_member_permissions') as string | null) || null;

			if (!isSuperAdmin) {
				const result = await setBuiltInCommandOverride(db, params.serverId, parseInt(id), {
					default_member_permissions: defaultMemberPermissions,
				});

				if (!result.success) {
					return fail(500, {
						error: result.error || 'Failed to update built-in command permissions',
					});
				}
					...
					for (let j = 0; j < choiceNames.length; j++) {
						if (choiceNames[j] && choiceValues[j]) {
							option.choices.push({
								...
								value:
									type === 4
										? parseInt(choiceValues[j] as string)
										: (choiceValues[j] as string),
							});
						}
					}
				}
	...
	delete: async ({ request, platform, params }) => {
		...
		try {
			const result = await deleteCommand(db, parseInt(params.commandId), guildId);
