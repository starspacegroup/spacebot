...
export const actions = {
  toggle: async ({ request, platform }) => {
    ...
    try {
      let result;
      if (isBuiltIn) {
        result = await setBuiltInCommandOverride(db, guildId, parseInt(id as string), { enabled });
      } else {
        result = await updateCommand(db, parseInt(id as string), { enabled });
      }
  ...
  delete: async ({ request, platform }) => {
    ...
    try {
      const result = await deleteCommand(db, parseInt(id as string), guildId);
