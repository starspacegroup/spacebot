<script lang="ts">
	...
	const summary = $derived(
		data?.summary ?? { totalGuilds: 0, totalMembers: 0, totalChannels: 0 }
	);
	...
	async function updateFirstLoginDmSetting(enabled) {
		...
		try {
			...
			const result = await parseJsonResponse(
				response,
				'Failed to update first-login DM setting'
			);
	...
	async function runCronJob(jobName, dangerous = false) {
		...
		if (dangerous) {
			const confirmed = confirm(
				'⚠️ This will delete all aggregated statistics and rebuild them from scratch.\n\nThis may take several minutes for large servers.\n\nAre you sure you want to continue?'
			);
			if (!confirmed) return;
		}
	...
	function startEditing(command) {
		editingCommand = {
			...command,
			response_embed: command.response_embed
				? JSON.parse(JSON.stringify(command.response_embed))
				: null,
		};
		commandError = null;
	}
	...
	async function deleteCommand(command) {
		if (
			!confirm(
				`Are you sure you want to delete the /${command.name} built-in command? This cannot be undone.`
			)
		)
			return;
	...
	{:else}
				...
				<div class="bot-info-card">
					...
					<div class="bot-details">
						...
						<div class="bot-badges">
							<span
								class="badge {botApp.isPublic ? 'badge-success' : 'badge-warning'}"
							>
		...
		<section class="stats-section superadmin-settings-section">
			<div class="superadmin-settings-header">
				...
				<div class="superadmin-settings-actions">
					<span
						...
						class:enabled={firstLoginDmEnabled}
						class:disabled={!firstLoginDmEnabled}
					>
						{firstLoginDmEnabled ? 'First-Login DMs On' : 'First-Login DMs Off'}
					</span>
					<button
						class="btn btn-sm {firstLoginDmEnabled ? 'btn-danger' : 'btn-primary'}"
						onclick={toggleFirstLoginDmSetting}
						disabled={firstLoginDmSaving}
					>
			...
			<p class="gateway-logs-hint">
				When enabled, any Discord OAuth user signing into SpaceBot for the first time
				triggers a DM to every configured superadmin account.
			</p>
			...
			{#if firstLoginDmError}
				<div class="cmd-toast cmd-toast-error">
					<span>✗</span>
					{firstLoginDmError}
					<button class="cmd-toast-close" onclick={() => (firstLoginDmError = null)}
						>✕</button
					>
				</div>
			{/if}
				...
				{#each cronJobs as job}
					<div class="cron-job-card {job.dangerous ? 'cron-job-dangerous' : ''}">
						...
						<div class="cron-job-details">
							...
							<div class="cron-detail">
								...
								<span class="cron-value">
									...
									{#if job.lastStatus}
										<span class="status-badge status-{job.lastStatus}"
											>{job.lastStatus}</span
										>
									{/if}
								</span>
							...
							{#if job.lastDuration}
								<div class="cron-detail">
									...
									<span class="cron-value"
										>{formatDuration(job.lastDuration)}</span
									>
								</div>
							{/if}
						</div>
						...
						{#if jobResults[job.name]}
							<div
								class="cron-result {jobResults[job.name].success
									? 'result-success'
									: 'result-error'}"
							>
									...
									<tr>
										<td class="job-name-cell">
											{cronJobs.find((j) => j.name === entry.job_name)
												?.displayName || entry.job_name}
										</td>
										<td>
											<span
												class="trigger-badge trigger-{entry.triggered_by}"
											>
												{entry.triggered_by}
											</span>
										</td>
										<td>
											...
											{#if entry.status === 'running'}
												<button
													...
													onclick={() =>
														forceCancelJob(entry.id, entry.job_name)}
		...
		<section class="builtin-commands-section">
			...
			{#if commandSuccess}
				<div class="cmd-toast cmd-toast-success">
					<span>✓</span>
					{commandSuccess}
					<button class="cmd-toast-close" onclick={() => (commandSuccess = null)}
						>✕</button
					>
				</div>
			{/if}
							...
							<div class="command-edit-form">
								<div class="form-row">
									...
									<div class="input-with-prefix">
										...
										<input
											type="text"
											bind:value={editingCommand.name}
											class="form-input"
								...
								<div class="form-row">
									...
									<select
										bind:value={editingCommand.response_type}
										class="form-select"
									>
								...
								{#if editingCommand.response_type === 'embed'}
									<div class="form-row">
										...
										<input
											...
											oninput={(e) => {
												if (!editingCommand.response_embed)
													editingCommand.response_embed = {};
												editingCommand.response_embed.title = (
									...
									<div class="form-row">
										...
										<textarea
											...
											oninput={(e) => {
												if (!editingCommand.response_embed)
													editingCommand.response_embed = {};
												editingCommand.response_embed.description = (
								...
								<div class="form-row form-row-inline">
									<label class="form-checkbox">
										<input
											type="checkbox"
											bind:checked={editingCommand.ephemeral}
										/>
						...
						<div class="command-card {command.enabled ? '' : 'disabled'}">
							...
							<div class="command-body">
								...
								<div class="command-meta">
									<span class="meta-tag">
										{responseTypes[command.response_type]?.label ||
							...
							<div class="command-footer">
								<div class="command-stats">
									<span class="stat-mini" title="Times used"
										>🔄 {command.use_count || 0}</span
									>
								</div>
						...
						{#if newCommand.response_type === 'embed'}
							<div class="form-row">
								...
								<input
									...
									oninput={(e) => {
										if (!newCommand.response_embed)
											newCommand.response_embed = {};
										newCommand.response_embed.title = (
											e.target as HTMLInputElement
										).value;
									}}
									class="form-input"
							...
							<div class="form-row">
								...
								<textarea
									...
									oninput={(e) => {
										if (!newCommand.response_embed)
											newCommand.response_embed = {};
										newCommand.response_embed.description = (
											e.target as HTMLTextAreaElement
										).value;
									}}
									class="form-textarea"
			...
			{:else}
				<button
					class="btn btn-primary add-command-btn"
					onclick={() => (showNewCommandForm = true)}
				>
						...
						<tbody>
							{#each guilds as guild}
								<tr>
									...
									<td class="numeric">
										{guild.stats?.member_count || guild.approximate_member_count
											? formatNumber(
													guild.stats?.member_count ||
									...
									<td class="numeric">
										{#if guild.stats?.boost_level}
											<span class="boost-level"
												>Level {guild.stats.boost_level}</span
											>
										{:else}
											—
										{/if}
					...
					{#each globalStats.recentActivityByGuild as activity}
						...
						<div class="activity-item">
							...
							<span class="activity-count"
								>{formatNumber(activity.event_count)} events</span
							>
						</div>
					{/each}
				</div>
...
<style>
	...
	.spinner-small {
		width: 14px;
		height: 14px;
		border: 2px solid var(--color-fixed-border-dark-soft);
