<script>
  ...
  function parseModelChain(raw) {
    ...
    for (const line of lines) {
      const [providerRaw, modelRaw, viaRaw] = line.split(":").map((part) => String(part || "").trim());
...
<div class="ops-page">
  ...
  <section class="ops-grid stats-grid">
    ...
    <article>
      ...
      <span>{runnerJobs.filter((job) => job.status === "pending" || job.status === "running").length} active</span>
    </article>
  </section>
  ...
  <section class="panel">
    ...
    <div class="form-grid">
      ...
      <select class="input" bind:value={formTargetInstanceId}>
        ...
        {#each runnerInstances as instance (instance.id)}
          <option value={instance.id}>{instance.display_name} {instance.is_online ? "(online)" : "(offline)"}</option>
        {/each}
      </select>
      ...
      <select class="input" multiple bind:value={formAllowedInstanceIds}>
        {#each runnerInstances as instance (instance.id)}
          <option value={String(instance.id)}>Allow instance: {instance.display_name}</option>
        {/each}
      </select>
    </div>
    ...
    <div class="capability-row">
      ...
      <label><input type="checkbox" bind:checked={formRequireCopilot} /> require copilot bridge</label>
    </div>

    <button class="btn" onclick={createWorkflow} disabled={creatingWorkflow || !formName.trim()}>
        ...
        {#each workflows as workflow (workflow.id)}
          <article class="workflow-card">
            <div>
              ...
              <p class="meta">{workflow.job_type} • {workflow.runner_strategy || "pinned"} • {workflow.model_preferences_json?.mode || "first_success"}</p>
            </div>
            <div class="workflow-actions">
              <label>
                <input
                  ...
                  onchange={(event) => toggleWorkflow(workflow, event.currentTarget.checked)}
                />
                {workflow.enabled ? "Enabled" : "Disabled"}
              </label>
              <button class="btn btn-danger" onclick={() => deleteWorkflow(workflow.id)}>Delete</button>
            </div>
          </article>
        {/each}
      {/if}
  ...
  <section class="panel">
    ...
    <div class="dispatch-grid">
      ...
      <input class="input" type="text" placeholder="Command override (shell workflows)" bind:value={dispatchCommand} />
      <input class="input" type="text" placeholder="Label override" bind:value={dispatchLabel} />
      <input class="input" type="text" placeholder='Payload JSON (optional)' bind:value={dispatchPayload} />
      <button class="btn" onclick={dispatchWorkflow} disabled={dispatching || !dispatchWorkflowId}>
        ...
        <tbody>
          {#each runnerJobs.slice(0, 80) as job (job.id)}
            <tr>
              ...
              <td>{job.claimed_by_instance_name || job.target_instance_name || job.runner_name || "-"}</td>
              <td>{job.attempt_count}/{job.max_attempts}</td>
              <td>{formatDate(job.updated_at || job.created_at)}</td>
              <td>
                {#if job.status === "pending" || job.status === "running"}
                  <button class="btn btn-danger" onclick={() => updateJob(job.id, "cancel")}>Cancel</button>
                {:else if job.status === "failed" || job.status === "canceled"}
                  <button class="btn" onclick={() => updateJob(job.id, "retry")}>Retry</button>
                {:else}
