...
async function touchRunnerToken(db, tokenId, clientIp) {
	...
	await db
		...
		.bind(clientIp ?? null, tokenId, `-${TOKEN_HEARTBEAT_WRITE_WINDOW_SECONDS} seconds`)
		.run();
}
...
async function storeResult(
	...
	instanceId,
	jobId,
	{ status, output, exitCode, resultJson, artifactRefs }
) {
	...
	server.addEventListener('message', async (event) => {
		...
		if (msg.type === 'hello') {
			try {
				...
				const instance = await registerRunnerInstance(
					db,
					auth,
					msg.instance ?? {},
					clientIp
		...
		if (msg.type === 'disconnect') {
			...
			try {
				if (msg.instance?.instanceKey && msg.instance?.displayName) {
					const refreshed = await registerRunnerInstance(
						db,
						auth,
						msg.instance,
						clientIp
		...
		if (msg.type === 'result') {
			...
			try {
				const persistedArtifactRefs = await persistArtifacts(
					db,
					auth,
					state,
					jobId,
		...
		while (!closed) {
			try {
				...
				if (jobs.length > 0) {
					...
					idlePollDelay = Math.min(
						Math.round(idlePollDelay * 1.5),
