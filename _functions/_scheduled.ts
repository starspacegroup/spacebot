...
async function recordJobStart(db, jobName, cronPattern) {
    ...
    `
			...
			.bind(jobName, cronPattern, startedAt)
			.run();
    ...
    `
		...
		console.error('[Scheduled] Failed to record job start:', error);
		return null;
	}
}
...
async function recordJobComplete(
    ...
    `
		...
		console.error('[Scheduled] Failed to record job completion:', error);
	}
}
  ...
  `
	...
	for (const event of events.results) {
      ...
      `
				...
				.bind(event.created_at, event.created_at, guildId, event.actor_id)
				.run();
			sessionsClosed++;
...
async function buildHourlyStats(db, guildId) {
    ...
    `
      ...
      `
			...
			startTime =
				earliest?.earliest || new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
		}
    ...
    `
      ...
      `
				...
				.bind(guildId, period.period_start)
				.first();
...
async function buildDailyStats(db, guildId) {
    ...
    `
			...
			.bind(guildId, lastAggregated?.last_period)
			.all();
...
async function runHourlyAggregation(env) {
	...
	console.log(
		`[Scheduled] Hourly aggregation completed: ${results.guildsProcessed} guilds, ${results.totalHourlyPeriods} hourly periods in ${duration}ms`
	);
...
async function runDailyRefresh(env) {
	...
	const staleVoiceResult = await db
		.prepare(
			`
    UPDATE voice_sessions
  ...
  `
	...
	await db
		.prepare(`DELETE FROM server_stats WHERE recorded_at < datetime('now', '-90 days')`)
		.run();

	const duration = Date.now() - startTime;
	console.log(
		`[Scheduled] Daily refresh completed: ${results.processed} guilds in ${duration}ms`
	);
 ...
 */
export async function onScheduled(event, env, _ctx) {
	...
	try {
		if (cronPattern === '0 * * * *') {
			...
			await recordJobComplete(
				...
				jobId,
				true,
				{ aggregation: aggregationResult, refresh: refreshResult },
				null,
				Date.now() - startTime
			);
			console.log(
				'[Scheduled] Daily job results:',
				JSON.stringify({ aggregation: aggregationResult, refresh: refreshResult })
			);
		} else {
			await recordJobComplete(
				db,
				...
				false,
				null,
				`Unknown cron pattern: ${cronPattern}`,
				Date.now() - startTime
			);
